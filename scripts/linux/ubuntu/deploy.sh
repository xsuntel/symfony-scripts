...
setPostgreSQL() {
  ...
  if [ -f ${PROJECT_PATH}/scripts/linux/ubuntu/etc/postgresql/_install.sh ]; then
    source ${PROJECT_PATH}/scripts/linux/ubuntu/etc/postgresql/_install.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/linux/ubuntu/etc/postgresql/_install.sh" && exit
  fi
  echo
}
