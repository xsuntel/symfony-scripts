...
setVM() {
  ...
  # >>>> Linux - Ubuntu - Status
  if [ -f ${PROJECT_PATH}/scripts/linux/ubuntu/_status.sh ]; then
    source ${PROJECT_PATH}/scripts/linux/ubuntu/_status.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/linux/ubuntu/_status.sh" && exit
  fi
  echo
}
