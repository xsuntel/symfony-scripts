...
| Script | Role |
|--------|------|
| `base/app/base-check.sh` | Repository runtime-artifact cleanup (removes `node_modules`, `vendor`, `var`, etc. — destructive, manual-run only) |
| `base/app/cache-clear.sh` | Dev environment rebuild (composer install, cache:clear, asset-map:compile — manual-run only) |
