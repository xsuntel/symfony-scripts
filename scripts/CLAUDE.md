...
| Script | Role |
|--------|------|
| `base/base-check.sh` | 저장소 런타임 산출물 정리 (`node_modules`, `vendor`, `var` 등 삭제 — 파괴적, 수동 실행 전용) |
| `base/cache-clear.sh` | 개발 환경 재빌드 (composer install, cache:clear, asset-map:compile — 수동 실행 전용) |
