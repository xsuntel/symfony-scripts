...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
  ...
  if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
    ...
    if [ -f /etc/nginx/nginx.conf ]; then
      sudo cp -fv ${PROJECT_PATH}/scripts/develop/linux/ubuntu/server/nginx/nginx_${ENVIRONMENT_NAME}.conf /etc/nginx/nginx.conf
    fi
    ...
    sudo cp -fv ${PROJECT_PATH}/scripts/develop/linux/ubuntu/server/nginx/conf.d/symfony_${ENVIRONMENT_NAME}.conf /etc/nginx/conf.d/symfony.conf
