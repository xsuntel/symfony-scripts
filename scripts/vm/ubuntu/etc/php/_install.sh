...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
  ...
  if [ "${PHP_FPM_STATUS}" != "php${PHP_VERSION}-fpm" ]; then
    (
      ...
      local addPackageList="php${PHP_VERSION}-opcache php${PHP_VERSION}-redis php${PHP_VERSION}-pgsql"
  ...
  if [ -f /etc/php/${PHP_VERSION}/cli/php.ini ]; then
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/cli/php_${ENVIRONMENT_NAME}.ini /etc/php/${PHP_VERSION}/cli/php.ini
  fi
  echo
  ...
  if [ -f /etc/php/${PHP_VERSION}/fpm/php.ini ]; then
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/fpm/php_${ENVIRONMENT_NAME}.ini /etc/php/${PHP_VERSION}/fpm/php.ini
  fi

  # >>>> PHP-FPM - php-fpm.conf
  if [ -f /etc/php/${PHP_VERSION}/fpm/php-fpm.conf ]; then
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/fpm/php-fpm_${ENVIRONMENT_NAME}.conf /etc/php/${PHP_VERSION}/fpm/php-fpm.conf
  fi

  # >>>> PHP-FPM - pool.d/www.conf
  if [ -f /etc/php/${PHP_VERSION}/fpm/pool.d/www.conf ]; then
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/fpm/pool.d/www_${ENVIRONMENT_NAME}.conf /etc/php/${PHP_VERSION}/fpm/pool.d/www.conf
  fi

  # >>>> PHP-FPM - conf.d/10-opcache.ini
  if [ -f /etc/php/${PHP_VERSION}/fpm/conf.d/10-opcache.ini ]; then
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/fpm/conf.d/10-opcache_${ENVIRONMENT_NAME}.ini /etc/php/${PHP_VERSION}/fpm/conf.d/10-opcache.ini
  fi

  # >>>> PHP-FPM - conf.d/20-xdebug.ini
  if [ "${ENVIRONMENT_NAME}" == "dev" ]; then
    if [ -f /etc/php/${PHP_VERSION}/fpm/conf.d/20-xdebug.ini ]; then
      sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/php/${PHP_VERSION}/fpm/conf.d/20-xdebug_${ENVIRONMENT_NAME}.ini /etc/php/${PHP_VERSION}/fpm/conf.d/20-xdebug.ini
    fi
