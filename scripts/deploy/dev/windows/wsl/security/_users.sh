...
#set -euo pipefail
# ----------------------------------------------------------------------------------------------------------------------
# Scripts - Deploy - Dev - Windows - WSL - Security - users
# ----------------------------------------------------------------------------------------------------------------------
# >>>> Platform
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    ...
    if [ "${ENVIRONMENT_NAME}" == "dev" ]; then
      ...
      if ! grep -q "umask 022" "${PROFILE_PATH}"; then
        # tee rather than `bash -c "cat >> ${PROFILE_PATH}"` — the nested shell re-parsed the path,
        # so any space in it would have split into two arguments and appended to the wrong file.
...
EOF
      ...
      # >>>> User - Application
      local delUserList="list sync games news uucp uuidd irc speech-dispatcher"
      ...
      # >>>> User - Network
      local delUserList="ftp tcpdump snmp snmpd mail proxy sssd"
      for userItem in ${delUserList}; do
        local USER_LIST
        USER_LIST=$(awk -F: '{print $1}' /etc/passwd | grep -i "${userItem}")
        if [ "${USER_LIST}" == "${userItem}" ]; then
      ...
      # >>>> User - Full-Stack
      local delUserList="redis postgres rabbitmq epmd"
      for userItem in ${delUserList}; do
        local USER_LIST
        USER_LIST=$(awk -F: '{print $1}' /etc/passwd | grep -i "${userItem}")
        if [ "${USER_LIST}" == "${userItem}" ]; then
