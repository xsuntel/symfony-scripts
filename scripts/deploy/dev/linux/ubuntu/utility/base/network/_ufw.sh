...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    ...
    if [ "${ENVIRONMENT_NAME}" == "dev" ]; then
      ...
      # >>>> Update allowed ports for Tools    - Docker - App
      sudo ufw allow from 172.17.1.0/24
      sudo ufw allow from 172.17.2.0/24
      # >>>> Update allowed ports for Tools    - Docker - Dev
      sudo ufw allow from 172.17.11.0/24
      sudo ufw allow from 172.17.21.0/24
