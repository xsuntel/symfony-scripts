...
setPlatform() {
  ...
  # >>>> Linux - Network
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_network.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_network.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/ubuntu/tools/_network.sh" && exit
  fi
  echo

  # >>>> Linux - Packages
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_packages.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_packages.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/ubuntu/tools/_packages.sh" && exit
  fi
  echo

  # >>>> Linux - Security
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_security.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/ubuntu/tools/_security.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/ubuntu/tools/_security.sh" && exit
  fi
  echo
}
