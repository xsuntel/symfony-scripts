...
setPlatform() {
  ...
  # >>>> MacOS - Network
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/network/_base.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/network/_base.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/macos/device/network/_base.sh" && exit
  fi
  echo

  # >>>> MacOS - Packages
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/packages/_base.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/packages/_base.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/macos/device/packages/_base.sh" && exit
  fi
  echo

  # >>>> MacOS - Security
  if [ -f "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/security/_base.sh ]; then
    source "${PROJECT_PATH}"/scripts/deploy/dev/linux/macos/device/security/_base.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/linux/macos/device/security/_base.sh" && exit
  fi
  echo

  # >>>> MacOS - Tools
  if [ -f "${PROJECT_PATH}/scripts/deploy/dev/macos/device/tools/supervisor/_install.sh" ]; then
    source "${PROJECT_PATH}/scripts/deploy/dev/macos/device/tools/supervisor/_install.sh"
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/deploy/dev/macos/device/tools/supervisor/_install.sh" && exit
  fi
  echo
}
...
setProject() {
  ...
  # >>>> Git
  if [ -f "${PROJECT_PATH}/scripts/utility/git/base/_config.sh" ]; then
    source "${PROJECT_PATH}/scripts/utility/git/base/_config.sh"
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/utility/git/base/_config.sh" && exit
  fi
...
setDocker() {
  ...
  # >>>> Docker
  if [ -f "${PROJECT_PATH}"/scripts/utility/docker/base/_install.sh ]; then
    source "${PROJECT_PATH}"/scripts/utility/docker/base/_install.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/utility/docker/base/_install.sh" && exit
  fi
  echo

  # >>>> Docker - Containers
  if [ -f "${PROJECT_PATH}"/scripts/utility/docker/base/_deploy.sh ]; then
    source "${PROJECT_PATH}"/scripts/utility/docker/base/_deploy.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/utility/docker/base/_deploy.sh" && exit
  fi
  echo
}
