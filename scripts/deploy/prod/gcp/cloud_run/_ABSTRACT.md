...
sudo mkdir -p /var/www/xsun
sudo chown -R "${LOGNAME}":"${LOGNAME}" /var/www/xsun

cd /var/www/xsun && if [ -d kr ]; then rm -rf kr ; fi
```

* Download this project

```bash
git clone https://github.com/xsuntel/xsun-app.git App

cd kr && find ./scripts/ -type f -name "*.sh" -exec chmod 775 {} \;
```

* Deploy this project

```bash
./scripts/deploy/ncloud/instance/deploy.sh
```
