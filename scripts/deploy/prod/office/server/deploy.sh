...
setPostgreSQL() {
  ...
  # >>>> PostgreSQL
  if [ -f "${PROJECT_PATH}"/scripts/common/database/postgresql/_install.sh ]; then
    source "${PROJECT_PATH}"/scripts/common/database/postgresql/_install.sh
  else
    echo "Please check a file : ./scripts/common/database/postgresql/_install.sh" && exit
  fi
  echo
}
