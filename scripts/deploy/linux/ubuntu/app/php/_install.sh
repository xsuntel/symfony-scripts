...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
  ...
  if [ "${ENVIRONMENT_NAME}" == "dev" ]; then
    if [ -f /etc/php/${PHP_VERSION}/fpm/conf.d/20-xdebug.ini ]; then
      sudo cp -fv ${PROJECT_PATH}/scripts/deploy/linux/ubuntu/app/php/${PHP_VERSION}/fpm/conf.d/20-xdebug_${ENVIRONMENT_NAME}.ini /etc/php/${PHP_VERSION}/fpm/conf.d/20-xdebug.ini
    fi
