...
setDocker() {
  ...
  CONTAINER_ID=$(docker run -d --add-host=host.docker.internal:host-gateway --net dev_back-end -p "127.0.0.1:${DOCKERFILE_LOCALHOST_PORT}:8080" --name "${IMAGE_NAME}-prod" "${IMAGE_NAME}:${TAG_NAME}")
