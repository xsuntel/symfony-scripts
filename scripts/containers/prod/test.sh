...
setDocker() {
  ...
  IMAGE_NAME=${DOCKERFILE_IMAGE_NAME:-"app-php"}
  TAG_NAME=$(date +%Y.%m.%d)
  ...
  #CONTAINER_ID=$(docker run -d --add-host=host.docker.internal:host-gateway -p 8080:8080 --name "${IMAGE_NAME}-prod" "${IMAGE_NAME}:${TAG_NAME}")
  CONTAINER_ID=$(docker run -d --add-host=host.docker.internal:host-gateway --net dev_back-end --ip 172.21.0.11 -p 8080:8080 --name "${IMAGE_NAME}-prod" "${IMAGE_NAME}:${TAG_NAME}")
