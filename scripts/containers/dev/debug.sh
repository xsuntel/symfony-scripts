#!/bin/bash
# ======================================================================================================================
# Scripts - Docker - Containers - Dev - Debug
# ======================================================================================================================
...
setNginx() {
  ...
  if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    LOCAL_LISTEN_PORT=$(netstat -an | grep -i LISTEN | grep -v tcp6 | grep ".8880" | awk '{print $6}')
...
setDocker() {
  ...
  CONTAINER_ID=$(docker run -d --add-host=host.docker.internal:host-gateway -p 8880:8080 "${DOCKERFILE_IMAGE_NAME}:${DOCKERFILE_TAG_NAME}")
  ...
  #docker exec -it "${CONTAINER_ID}" timeout 1 bash -c 'cat < /dev/tcp/host.docker.internal/6379' && echo "Redis Connected" || echo "Redis Connection Failed"
  #echo
...
setVM() {
  ...
  if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    ...
    netstat -nlp | grep 8880
    echo

    echo ">>>> VM - Network - Port"
    echo
