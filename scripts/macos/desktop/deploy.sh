...
setRedis() {
  ...
  # >>>> Import an install file
  if [ -f ${PROJECT_PATH}/scripts/macos/desktop/etc/redis/_install.sh ]; then
    source ${PROJECT_PATH}/scripts/macos/desktop/etc/redis/_install.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/macos/desktop/etc/redis/_install.sh" && exit
  fi
  echo
}
...
setDocker() {
  ...
  if [ -f ${PROJECT_PATH}/scripts/macos/desktop/etc/postgresql-compose.${ENVIRONMENT_NAME}.yml ]; then
    echo ">>>> Docker Desktop - checking docker compose"
    echo
    cp -fv ${PROJECT_PATH}/scripts/macos/desktop/etc/postgresql-compose.${ENVIRONMENT_NAME}.* ${PROJECT_PATH}
    echo
  fi
