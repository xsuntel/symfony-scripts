...
setDocker() {
  ...
  if [ -f ${PROJECT_PATH}/scripts/windows/device/docker-compose.${ENVIRONMENT_NAME}.yml ]; then
    cp -fv ${PROJECT_PATH}/scripts/windows/device/docker-compose.${ENVIRONMENT_NAME}.* ${PROJECT_PATH}
    cp -fv ${PROJECT_PATH}/scripts/windows/device/docker-compose.yml ${PROJECT_PATH}
    echo
  fi
