...
setVM() {
  ...
  if [ -f ${PROJECT_PATH}/scripts/windows/device/_vm.sh ]; then
    source ${PROJECT_PATH}/scripts/windows/device/_vm.sh
  else
    echo "Please check a file : ${PROJECT_PATH}/scripts/windows/device/_vm.sh" && exit
  fi
}
