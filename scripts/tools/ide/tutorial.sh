...
setPhp() {
  ...
  if [ -f app/bin/console ]; then
    ...
    # Reached when ./app has no bin/console - a partial install, or a directory the user placed here.
    # Deleting that silently would discard unrelated work, so announce it and skip when there is nothing to remove.
    if [ -d app ]; then
      echo "[ Warning ] ./app exists but is not a Symfony project - removing it"
      rm -rfv app
    fi
  fi
  echo
