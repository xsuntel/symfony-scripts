...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
    ...
    if [ "${ENVIRONMENT_NAME}" == "dev" ]; then
        ...
        if [ ! -f /etc/supervisor/conf.d/messenger-consume.conf ]; then
            if [ -f "${PROJECT_PATH}/scripts/common/server/supervisor/conf.d/messenger-consume.conf" ]; then
                sudo cp -fv "${PROJECT_PATH}/scripts/common/server/supervisor/conf.d/messenger-consume.conf" /etc/supervisor/conf.d/messenger-consume.conf
                echo
            fi
        fi
