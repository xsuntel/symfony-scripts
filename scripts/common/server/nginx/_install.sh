...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
  ...
  if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
    ...
    if [ -f /etc/nginx/nginx.conf ]; then
      sudo cp -fv "${PROJECT_PATH}"/scripts/common/server/nginx/nginx.conf /etc/nginx/nginx.conf
    fi
    ...
    sudo cp -fv "${PROJECT_PATH}"/scripts/common/server/nginx/conf.d/symfony.conf /etc/nginx/conf.d/symfony.conf
    ...
    # Not pgrep: this is the form published in the Symfony doc linked above, and it needs the
    # user column, which pgrep does not report.
