    ...
    (
      ...
      if [ -f bin/console ]; then
        ...
        # --------------------------------------------------------------------------------------------------------------
          ...
          echo ">>>> consume"
          # Long-running workers accumulate memory (Doctrine profiling, UoW) and must be
          # restarted periodically — the worker exits on its own once a limit is reached.
          echo "[ INFO ] The worker exits on its own at --time-limit/--memory-limit. Re-run this menu to resume."
          echo
          symfony console messenger:consume --all -v --time-limit=3600 --memory-limit=256M
