...
if [ -d app ]; then
  (
    ...
    if [ -f bin/console ]; then
      ...
      # Absolute paths with :? so an unset PROJECT_PATH aborts instead of deleting from the cwd.
      if [ -d var/cache ]; then
        if [ "${PLATFORM_TYPE}" == "Linux" ]; then
          if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
            sudo rm -rf "${PROJECT_PATH:?}/app/var/cache"/*
          else
            rm -rf "${PROJECT_PATH:?}/app/var/cache"/*
          fi
        else
          rm -rf "${PROJECT_PATH:?}/app/var/cache"/*
        fi
      ...
      if [ -d var/log ]; then
        if [ "${PLATFORM_TYPE}" == "Linux" ]; then
          if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
            sudo rm -rf "${PROJECT_PATH:?}/app/var/log"/*
          else
            rm -rf "${PROJECT_PATH:?}/app/var/log"/*
          fi
        else
          rm -rf "${PROJECT_PATH:?}/app/var/log"/*
        fi
      ...
      if [ -d var/sessions ]; then
        if [ "${PLATFORM_TYPE}" == "Linux" ]; then
          if [ "${ENVIRONMENT_NAME}" == "prod" ]; then
            sudo rm -rf "${PROJECT_PATH:?}/app/var/sessions"/*
          else
            rm -rf "${PROJECT_PATH:?}/app/var/sessions"/*
          fi
        else
          rm -rf "${PROJECT_PATH:?}/app/var/sessions"/*
        fi
