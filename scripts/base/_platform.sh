...
if [ "${PLATFORM_TYPE}" == "Linux" ]; then
  ...
  local addPackageList="make acl curl git wget net-tools unzip vim"
  ...
  local addPackageList="libfreetype6-dev libpng-dev libmcrypt-dev libonig-dev libpq-dev libicu-dev libzip-dev libssl-dev"
  for pkgItem in ${addPackageList}; do
    local APT_PKG_INFO
    APT_PKG_INFO=$(dpkg -l | grep -i "${pkgItem}" | awk '{print $2}' | cut -d ':' -f1 | awk "/^${pkgItem}$/")
    if [ "${APT_PKG_INFO}" != ${pkgItem} ]; then
  ...
  if [ ! /etc/rsyslog.conf ]; then
    sudo apt-get install -y rsyslog
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/rsyslog.conf /etc/rsyslog.conf
  ...
  if [ "${SUPERVISOR_STATUS}" == "active" ]; then
    ...
    sudo cp -fv ${PROJECT_PATH}/scripts/vm/ubuntu/etc/supervisor/conf.d/messenger-worker.conf /etc/supervisor/conf.d/messenger-worker.conf
    echo
