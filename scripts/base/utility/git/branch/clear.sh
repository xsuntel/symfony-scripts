    ...
    (
      ...
      if [ -f bin/console ]; then
        ...
        if [ -d var ]; then
          ...
          # `|| true` keeps the cleanup going past a partial failure, but stderr is deliberately NOT
          # redirected - a permission error here leaves var/ half-cleaned and must stay visible.
          if [ -d var/cache ]; then
            find var/cache -mindepth 1 -delete || true
          fi
          ...
          find var -maxdepth 1 ! -name 'cache' ! -name 'log' ! -name 'var' -exec rm -rf {} + || true
        fi
