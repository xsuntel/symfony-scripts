...
├── .claude/                                 ← Claude Code config (agent-memory, agents, docs, hooks, output-styles, rules, scripts, skills, workflows, settings; `commands/` retired 2026-09-10)
...
`abstract-orchestrator-contract-docs.md` (the operational contract all four orchestrators execute), and
`abstract-agentic-harness-skill` (the meta-engineering design skill). That prefix is a deliberate part
of the convention, not a violation of it.

> **2026-09-09 — the three `abstract-*-engineering-skill` directories added on 2026-09-06 were merged
> into `abstract-agentic-harness-skill`**, by explicit user instruction. The three axes — *what to
> build* (harness), *how it converges* (loop), *how it connects* (graph) — were deliberately split, but
> every one of the three bodies opened with the same three-axis map and handed work to the other two, so
> a single design decision paid three cold starts. The axes survive **as sections**: they are now
> `references/{harness-design,loop-engineering,graph-engineering}-guide.md`, reached through a dispatch
> table in `SKILL.md`, with their section numbering preserved because other files cite it by number.
...
| `skills/` | User-invocable skills (`/…-skill`) that orchestrate the domain rules. **Flat tree** — Claude Code takes the command name from the directory, so the taxonomy is encoded as `<domain>-<name>-skill` (e.g. `tools-gcp-cloudrun-skill`). The `-skill` suffix is mandatory. **`abstract-` is the one permitted non-domain prefix**, carried by the meta-engineering skill `abstract-agentic-harness-skill` (added 2026-09-06 as three separate skills, merged into one 2026-09-09), which covers all three design axes — what artifacts to build, how a verification loop converges, and how nodes and edges connect. It is a **design skill, not a domain skill**: it holds no criteria and reaches the existing SoTs by `@see`, so it ships without the `rule + docs + reviewer-agent` quartet — see `## Skill Directory Layout` in `utility-claude-code-rule.md`. **A skill directory may carry supporting files**, and `references/*-guide.md` is the convention: the skill root is flat, the directories inside it are not, and a guide loads only when `SKILL.md` points at it. Twelve skills now carry a mode dispatch table naming which guide to read — a `## Modes` section in nine of them, and an extra row in the existing Part table of the three `app-*` skills. That table is what replaced the retired `commands/` tree |
| `commands/` | **Retired 2026-09-10 — empty, kept as a `.gitkeep` scaffold.** All 13 command files were folded into their paired skill as `references/*-guide.md`, and each of the 12 destination skills gained a mode dispatch table. `/app-php-symfony-review` is now the Review mode of `/app-php-symfony-skill`, and so on for the rest. Do not author a new file here — author a skill, or add a mode to an existing one. The SoT for the retirement and the full old→new invocation mapping is `## Commands Layout — retired tree` in `utility-claude-code-rule.md` |
| `agents/` | Sub-agents per domain — authors, analyzers (security), debuggers, reviewers, and testers, plus **four orchestrators**: `app-agent-team` (anchored **exclusively** on the 15 `app-*` agents, driving the pre-deploy gate and Commit through skills), `api-agent-team` (anchored **exclusively** on the 5 `api-platform-*` agents, scoped to the REST exposure layer), `tools-agent-team` (added 2026-08-30 — anchored on the five prefixes `cache-*`, `database-*`, `server-*`, `tools-aws-*`, `tools-gcp-*`, resolved by preflight each invocation, and carrying the **Review axis only**), and `utility-agent-team` (the cross-cutting utility surface — `diagram/**`, commit messages, `scripts/**`, `.claude/**` — which holds no criteria and **spawns no specialist directly**, delegating every job with the Skill tool to the utility skill that owns its own loop and retry budget). **Every roster is closed** — no orchestrator spawns a specialist outside its own prefix set. Consequently the six infrastructure reviewers split in two: **five are direct-spawned by `tools-agent-team`** (and are still reachable through the Review mode of their domain skill, or the `tools-app-deploy-skill` gate), while `message-rabbitmq-reviewer` matches none of the five prefixes and is reachable **only through the Review mode of `/message-rabbitmq-skill`**. **Flat tree** — named `<domain>-<name>.md` (e.g. `app-php-symfony-reviewer.md`), and the filename must equal the frontmatter `name`, which is also the `agent-memory/` directory key. All four orchestrators carry a domain prefix like every other agent |
...
`agents/`, `agent-memory/`, `docs/`, and `skills/` keep their current names. Do not rename,
...
> **`commands/` left that list on 2026-09-10**, when the user instructed the whole command tree be merged
> into `skills/` and confirmed the instruction named the three frozen `api-*` command paths. That is the
> carve-out the freeze clause provides, exercised deliberately — **no `api` identifier was renamed**, the
> two destination skill directories already existed, and every other frozen `api` entry is untouched.
