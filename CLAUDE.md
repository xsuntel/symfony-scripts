...
Domain expertise for this repository is codified under `.claude/`, organized by the same
`domain/base/…` taxonomy used throughout the project (`api`, `app`, `cache`, `database`, `message`,
`server`, `utility`). `.claude/rules/structure-rule.md` is the entry-point index and single source of
truth (SoT) for the rule and docs trees.

| Subdirectory | Purpose |
|---|---|
| `rules/` | Judgment criteria (SoT) per domain — auto-applied when editing files matched by each rule's `paths` frontmatter |
| `docs/` | Example/reference edition paired with each rule (annotated configs, code samples) |
| `skills/` | User-invocable helpers (`/…-review`, `/…-build`, `/…-client`, etc.) that orchestrate the domain rules |
| `commands/` | Slash-command entry points for the review skills |
| `agents/` | Sub-agents per domain — reviewers, authors, test writers, debuggers, and the `app-agent-team` orchestrator |
| `agent-memory/` | Per-agent project memory (`<domain>/<agent>/MEMORY.md`), consumed by agents whose frontmatter sets `memory: project` |
| `output-styles/` | Response-style presets; the active style is selected in `settings.json` (`outputStyle`) |
| `hooks/` | Lifecycle hook drop-ins (per-event directories); active hooks: `post-tool-use/php-cs-fixer.sh`, `stop/notify-complete.sh` |
...
register it in the `structure-rule.md` index.
