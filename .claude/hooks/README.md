...
- Prefix the script path with **`${CLAUDE_PROJECT_DIR}`**, the same way `settings.json` writes
  ...
  > ✅ **Done on 2026-09-13 — the convention is now met.** `[Verified]` 2026-09-13
  > [Read: .claude/settings.json] — all **14 `command` values** carry the prefix: the 13 hook commands
  > and `statusLine`.
  >
  > **Write it quoted: `"\"${CLAUDE_PROJECT_DIR}\"/.claude/hooks/<event>/<name>.sh"`.** The live page
  > says "Prefer [exec form] for any hook that references a path placeholder. In shell form, wrap each
  > placeholder in double quotes." `[Verified]` 2026-09-13
  > [WebFetch: <https://code.claude.com/docs/en/hooks>]. These 13 take no arguments, so shell form with
  > the placeholder quoted was chosen over adding `"args": []` to each — it is the documented-correct
  > form, and it changes path resolution without changing execution semantics. The quotes are what keeps
  > a repository path containing a space from splitting into two words.
  >
  > **History — this was an open task for eleven days, and had regressed once before that.** From
  > 2026-09-02 this note read: "of the 14 `command` values, only `statusLine` carries the prefix; all 13
  > hook commands are bare relative paths", correcting an earlier version of this paragraph that had
  > falsely asserted the opposite. The damage was never cosmetic —
  > `pre-tool-use/agent-roster-guard.sh` resolves `.claude/agents/<target>.md` to decide whether a spawn
  > target exists, so from any other working directory the guard reported every agent as missing.
  > `env.CLAUDE_BASH_MAINTAIN_PROJECT_WORKING_DIR: "1"` pins the *Bash tool's* working directory and
  > partly masked this, but it was never a guarantee for hook execution `[Uncertain]`.
  >
  > **The prefix does not retire the in-script fallback.** A hook that reads a project file needs the
  > prefix *and* an internal `PROJECT_PATH` fallback; `agent-roster-guard.sh` shows the pattern and must
  > keep it. Flag as a `[MUST]` any edit that drops either half, or that returns a hook command to a
  > bare relative path.
...
| --- | --- | --- |
| `PreToolUse` (`Agent\|Task`) | `pre-tool-use/agent-roster-guard.sh` | Enforces the closed-roster invariant of the four orchestrators — matches the calling `agent_type` against the spawn target's `subagent_type` and **blocks with exit 2** when a team reaches outside its own prefix set. Non-orchestrator callers pass through untouched |
