...
#
# Why this exists: the four orchestrators each carry an "only spawn my roster" invariant, but a subagent
...
#
# Judgment SoT: the roster invariant section of .claude/rules/{app,api,tools,utility}-agent-team-rule.md.
# Those files word that heading differently, so each case branch below carries its own RULE_SECTION rather
# than a single shared string. All four now resolve — utility-agent-team-rule.md was created 2026-09-13,
# closing the gap that had this branch cite the agent prompt instead since 2026-09-11.
...
# Only the four orchestrators carry a roster invariant. Anything else — the main thread, a domain agent
...
case "${CALLER}" in
  ...
  tools-agent-team)
    ...
    HANDOFF='message-rabbitmq -> the Review mode of /message-rabbitmq-skill under app-agent-team / deploy go-no-go -> tools-app-deploy-skill'
    ;;
  # This team delegates with the Skill tool and has no roster of its own. It holds Agent only because a
  # skill runs in its caller's context: all four utility skills say "call the <name> agent" in their
  # loop, so the orchestrator is the one making that call and cannot run them without it. The pattern
  # is therefore the 8 agents those skills own, and nothing else — deliberately narrower than a bare
  # utility- prefix, which would also match utility-agent-team itself. What this cannot check is *why*
  # the spawn happened: an improvised spawn of the same 8 agents, outside the skill procedure that owns
  # them, looks identical here and stays self-held by the agent prompt.
  # Widened 2026-09-11 from the drawio/git-commit pairs to include shell-script and claude-code, when
  # those two domains were given agent pairs and their skills handed over their self-verification loops.
  utility-agent-team)
    ALLOW_PATTERN='^utility-(drawio-diagram|git-commit|shell-script|claude-code)-(author|reviewer)$'
    ROSTER_DESC='the 8 utility-* agents, and only as a step of the skill that owns them'
    RULE_PATH='.claude/rules/utility-agent-team-rule.md'
    RULE_SECTION='## Invariant — the roster is closed and skill-scoped'
    HANDOFF='application code -> app-agent-team / exposure layer -> api-agent-team / infrastructure and deploy -> tools-agent-team'
    ;;
  *) exit 0 ;;
esac
