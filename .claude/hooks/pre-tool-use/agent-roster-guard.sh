...
#
# Judgment SoT: the roster invariant section of .claude/rules/{app,api,tools}-agent-team-rule.md. Those
# files word that heading differently, so each case branch below carries its own RULE_SECTION rather than a
# single shared string. utility-agent-team has no rule file yet (added 2026-09-11; the rule is a tracked
# follow-up), so its branch cites the agent prompt instead — a RULE_PATH that does not resolve would send
# a blocked caller to nothing.
...
case "${CALLER}" in
  ...
  utility-agent-team)
    ...
    RULE_PATH='.claude/agents/utility-agent-team.md'
    RULE_SECTION='## Team Roster (4 skills + the 8 agents they own)'
