...
- For database passwords containing special characters, prefer individual parameter variables over the URL format to avoid encoding issues.
...
- Allow XML only in exceptional cases where IDE autocompletion requires it.

## Environment Variables (infrastructure configuration)

Use environment variables for values that vary per machine (database credentials, external API URLs, ports).
...
- Secrets are encrypted when stored in `config/secrets/{env}/`.
