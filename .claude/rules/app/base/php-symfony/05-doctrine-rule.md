...
- Mark entity classes `final` unless it is an abstract base entity.
...
- Encapsulate all business queries in a Repository class — do not use DQL or QueryBuilder in a Controller or Service.
- Extend `Doctrine\ORM\EntityRepository` (do not use `ServiceEntityRepository` unless DI is required).
...
Declare `#[ORM\Index]` on every foreign-key column — Doctrine ORM 3 does not add these automatically.
...
- Bidirectional relationships must always declare both `mappedBy` and `inversedBy`.
- `OneToMany` collections must be initialized as an `ArrayCollection` in the constructor.
...
- Destructive changes (dropping a column, changing the type of a production column) require a two-step migration: (1) backfill/mark deprecated, (2) drop.
