...
- Define every form as a PHP class extending `AbstractType`.
- Do not define forms inline inside a controller.
...
Bind forms to a **DTO class**, not an Entity. This decouples the HTTP layer from the persistence model and prevents sensitive fields (e.g. `id`, `createdAt`) from being exposed.
...
Use the DTO in the controller, then map it to the Entity in a Service:
...
Specify the DTO class with the `data_class` option:
...
- This applies the constraints consistently regardless of how the object is populated (Form, API, CLI).
...
Use validation groups when different scenarios (create vs. edit) require different validation rules. The `Default` group always applies.
