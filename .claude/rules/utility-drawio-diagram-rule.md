...
@see .claude/skills/utility-drawio-diagram-skill/references/review-guide.md — standalone review procedure & output format for an existing `.drawio`
...
- Authoring and modification use the `utility-drawio-diagram-skill` skill (author→reviewer loop,
  max 2 retries).
- A standalone quality review of an existing file uses the `/utility-drawio-diagram-skill <file-path>`
  command.
