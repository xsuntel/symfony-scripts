...
@see .claude/docs/server/base/nginx-config-docs.md — annotated configs, file mapping, checklist
...
- The production Dockerfile copies only `http.d/www.conf` (line 128). When changing `nginx.conf`, uncomment line 127 and rebuild the image.
...
- Keep `fastcgi_read_timeout` shorter than PHP-FPM's `request_terminate_timeout` (120s for the production pool).
