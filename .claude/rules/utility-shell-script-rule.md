...
- Use only global variable names declared in `scripts/common/_abstract.sh` (catalog in the docs). Do not invent non-existent globals.
...
- `scripts/common/` (environment-independent install/config, sourced by both containers and deploy) / `scripts/containers/{dev,prod}/` / `scripts/deploy/{dev,prod}/`. Details in the docs.
