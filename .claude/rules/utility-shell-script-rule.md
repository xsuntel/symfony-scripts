...
- **The brace requirement admits no special-parameter exemption** — `${REPLY}`, `${1}`, `${file}` are braced like any other name — and it is lint-enforced: `scripts/.shellcheckrc` sets `enable=require-variable-braces`, so an unbraced expansion fails the merge gate rather than relying on a reviewer to catch it.
- **One expansion is legitimately braced but unquoted:** `for x in ${LIST}` relies on word splitting to iterate a space-separated list (`_crontab.sh`, the `for pkgItem in ${addPackageList}` install loop). Add braces, never quotes — quoting collapses the loop to a single iteration. Everywhere else, braced *and* quoted.
...
- New scripts use the `utility-shell-script-skill` skill (author→reviewer loop); quality review of an existing file uses the `/utility-shell-script-skill` skill.
