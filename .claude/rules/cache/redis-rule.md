...
- Use `kernel.debug` for environment-aware TTLs: short values in dev, production values in prod.
...
**Session store** (`RedisSessionHandler` — `config/services.yaml:100-103`):
...
- Always inject the named lock store that matches the current domain — do not use the default lock store.
