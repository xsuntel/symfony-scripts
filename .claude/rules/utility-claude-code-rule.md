...
(agent · skill · command · rule · output style · settings · CLAUDE.md) are owned by the artifact spec guide
below; this rule does not restate them. It owns only the structural criteria.

@see .claude/skills/utility-claude-code-skill/references/artifact-spec-guide.md — per-type frontmatter & verification checklist (SoT; was `commands/utility-claude-code-review.md` until 2026-09-10)
...
`agents/`, `agent-memory/`, `output-styles/` — were a **single tier** that encodes the domain taxonomy as
...
**Six live trees as of 2026-09-10.** `commands/` was emptied on that date by explicit user instruction —
its 13 files were folded into their paired skill as `references/*-guide.md` — and the directory survives
as a `.gitkeep` scaffold. It is still listed above because the flat convention is what its files followed
while it was live; see `## Commands Layout — retired tree`.

Per-tree naming criteria live in the sections below: `## Rules Layout`, `## Docs Layout`,
`## Skill Directory Layout`, `## Commands Layout — retired tree`, `## Agents Layout`,
`## Agent Memory Layout`, `## Output Style Layout`.
...
the four `*-review` slash commands were **user-facing invocations that changed** (and changed again on
2026-09-10, when the whole tree folded into `skills/` — see `## Commands Layout — retired tree`), and the six reviewer
...
- **Encode the taxonomy in the directory name**, joining the path segments with hyphens and ending
  ...
  directory missing the suffix is a `[MUST]` finding in `/utility-claude-code-skill`.
- **`abstract-<name>-skill` is the one permitted non-domain prefix**, on exactly the footing
  ...
  domain prefix it does not belong to. **One exists as of 2026-09-09** —
  `abstract-agentic-harness-skill`, which covers all three design axes (what artifacts to build, how a
  verification loop converges, how nodes and edges connect) as `references/*-guide.md` sections. Do not
  "fix" it into a domain-prefixed name, and do not read the prefix as a taxonomy violation. Note what
  the carve-out does **not** relax: the `-skill` suffix, the ≤ 64-char `name`, and `name:` equalling the
  directory name all still apply.

  **2026-09-09 — the three merged into one, by explicit user instruction.** They were created on
  2026-09-06 as `abstract-harness-engineering-skill`, `abstract-loop-engineering-skill` and
  `abstract-graph-engineering-skill`, on the reasoning that the three axes never restate one another.
  That held, but it turned out to be the wrong unit for an **entry point**: each body opened with the
  same three-axis map and handed work to the other two, so one design decision triggered three skills
  and paid three cold starts. The merge keeps the boundaries (each axis is its own reference guide with
  its section numbering intact) and collapses only the invocation. **This is the carve-out both
  `## Prohibitions — Structure Immutability` and the deletion clause below require** — an explicit
  instruction naming the source and target directories — not a consistency cleanup.
...
- **A skill directory may carry supporting files, and `references/` is the convention for them.** The
  skill *root* is flat; the directories inside it are not constrained. `[Verified]` 2026-09-10
  [WebFetch: <https://code.claude.com/docs/en/skills>]: "Skills can include multiple files in their
  directory… Keep `SKILL.md` under 500 lines. Move detailed reference material to separate files," and a
  supporting file loads **only when `SKILL.md` points Claude at it**, so it costs nothing until read.
  Twelve skills use this: the seven `references/*-guide.md` in `abstract-agentic-harness-skill` (plus its
  `examples/`), the four `provider-registry.md` files, and the 13 `*-guide.md` files the retired
  `commands/` tree folded into on 2026-09-10. **Name a guide for its mode, not its origin**
  (`review-guide.md`, `build-guide.md`, `artifact-spec-guide.md`), and **always reach it from a mode
  dispatch table in `SKILL.md`** (a `## Modes` section, or an existing Part table extended with a row) — nothing resolves a `references/` path automatically, so an unlinked
  guide is dead weight and a stale pointer fails silently at read time.
...
- **A cross-cutting orchestration skill has no quartet, and that is not a gap.** `CLAUDE.md` requires a
  ...
  **Two such skills exist as of 2026-09-09**, in two shapes:
  - `tools-app-deploy-skill` — a pre-deploy gate that delegates to `server-nginx-reviewer`,
    `tools-gcp-cloudrun-reviewer`, `tools-aws-ecs-reviewer` and `/utility-shell-script-skill`, so it
    deliberately has no `tools-app-deploy-rule.md`, no `-docs.md` and no reviewer agent.
  - `abstract-agentic-harness-skill` — the merged harness · loop · graph design skill. It is a **design
    skill, not a domain skill**: it explicitly disclaims holding criteria and reaches the existing SoTs
    (`utility-claude-code-rule.md`, `skills/utility-claude-code-skill/references/artifact-spec-guide.md`,
    ...
    pays for repeatedly — so its absence is the design, not an omission.

  Do not flag either of the two for missing quartet members — the same carve-out `## Docs Layout`
...
- **Deleting a docs file is still prohibited** without an explicit instruction. An empty stub
  (`[Verified]` 2026-09-11 — `utility-git-commit-docs.md` is the only remaining 0-byte file;
  `utility-claude-code-docs.md` has since been filled) is a deliberate placeholder tracked in
  `TODO.md`, not a file to clean up.

## Commands Layout — retired tree

**`.claude/commands/**` is empty as of 2026-09-10, by explicit user instruction**, and the directory is
kept only as a `.gitkeep` scaffold. All 13 command files were folded into their paired skill as a
`references/*-guide.md`, and each of the 12 destination skills gained a mode dispatch table naming which
guide to read — a `## Modes` section in nine, and an extra row in the existing Part table of the three
`app-*` skills, which already had one. **Do not author a new file here** — author a skill, or add a mode to an existing one.

The merge rests on the spec, not on a preference. `[Verified]` 2026-09-10
[WebFetch: <https://code.claude.com/docs/en/skills>]: "Custom commands have been merged into skills. A
file at `.claude/commands/deploy.md` and a skill at `.claude/skills/deploy/SKILL.md` both create
`/deploy` and work the same way." The same page confirms a `SKILL.md` accepts `argument-hint`,
`allowed-tools` and `$ARGUMENTS` substitution, and documents `references/` supporting files as the way to
keep a `SKILL.md` under 500 lines — which is what made the fold lossless.

- **The invocations changed, and that was the cost paid.** `/app-php-symfony-review` is now the Review
  mode of `/app-php-symfony-skill`; `/utility-claude-code-review` is the Review mode of
  `/utility-claude-code-skill`, and so on. The full old→new table is in the `## Modes` block of each
  destination skill. A stale `/…-review` reference is not a broken link — **it is an instruction to type
  a command that no longer exists**, the same defect class as a stale `/domain:name` reference.
- **Two invocations collapsed into one.** `/api-platform-review` and `/api-platform-rest-build` both
  resolve to `/api-platform-rest-build-skill`, which is why that skill's `## Modes` table has three rows
  and why a caller **must name the mode** — Build writes to `app/src/**`, Review does not write at all.
- **Three deliberate author-vs-review file boundaries became mode boundaries.** The
  `utility-claude-code`, `utility-drawio-diagram` and `utility-shell-script` domains each had a review
  command split off from an authoring skill; the `## Modes` table is now the only thing keeping those
  two roles apart. **Flag as a `[MUST]` any edit that lets a Review mode write to its target.**
- **The directory itself must not be deleted** — `## Prohibitions — Structure Immutability` forbids
  deleting a directory, and the `.gitkeep` is deliberate, like the `.claude/hooks/**` scaffolds.
- **A command file is still a valid Claude Code artifact type**, and the spec for one is still carried in
  `skills/utility-claude-code-skill/references/artifact-spec-guide.md`. Not using them here is a
  **project convention**; do not read the retirement as a claim that the feature was removed.

**History, before the retirement.** The tree was flattened on 2026-08-15. A slash command has no `name`
frontmatter — the path *was* the invocation name — so nesting was not neutral:
`commands/app/php-symfony-review.md` invoked as `/app:php-symfony-review`, with the directory becoming a
`:`-separated namespace. Flattening therefore changed every command's invocation name, and file names
were `<domain>-<name>-review.md` with the filename minus `.md` being exactly what the user typed.
...
| ~~`.claude/commands/api-*.md`~~      | ~~3~~ — **row retired 2026-09-10**             | folded into `skills/api-platform-*-build-skill/references/`; see the note below |
...
> domain"). Both were `api-*` command files and both were frozen from the moment they were created —
> `CLAUDE.md` had always said so, freezing "every `api-*` entry in … `commands/`" — so this is a
> bookkeeping fix to the table, **not** a new freeze. `[Verified]` 2026-08-25: `.claude/commands/`
> contained exactly three `api-*` files, and the row listed all three.
>
> **2026-09-10 — the commands row is retired, under the explicit-instruction carve-out.** The user
> instructed that `@.claude/commands/*` be merged into `.claude/skills/**` and confirmed, when asked
> directly, that the glob names the three frozen `api-*` command paths. All three moved:
> `api-platform-review.md` → `skills/api-platform-rest-build-skill/references/review-guide.md`,
> `api-platform-rest-build.md` → `skills/api-platform-rest-build-skill/references/build-guide.md`,
> `api-platform-oauth2-build.md` → `skills/api-platform-oauth2-build-skill/references/build-guide.md`.
>
> **This is the carve-out operating as written, not an erosion of the freeze.** The clause requires "an
> explicit user instruction naming the `api` paths"; the instruction was checked against that bar before
> the move rather than assumed. Note what did **not** change: `api-platform-rule.md`, the six
> `api-platform-*` agents, their memory directories, `api-platform-docs`, `api-agent-team-docs` and the
> five `api-*-skill` directories are untouched and remain frozen. The two destination skill directories
> already existed and kept their names, so **no `api` identifier was renamed** — only three files moved
> into them. Do not read this precedent as licence to restructure anything else in the domain.
>
...
> conventions** — `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` remain SoT for those, so
...
- **Both authoring and review run through `utility-claude-code-skill`**, which dispatches on its
  `## Modes` table: the **Author** mode writes an artifact after a two-agent
  `utility-claude-code-author` → `utility-claude-code-reviewer` loop (max 2 REDO retries, writing only on
  a `Verdict: PASS`), the **Review** mode judges an existing artifact and writes nothing. Until
  2026-09-10 the review half was the separate `/utility-claude-code-review` command, and until
  2026-09-11 the Author half **self-verified inside the skill** with no downstream reviewer to defer to.
  Through both changes the criteria did not move, only the entry point and the owner of the verdict —
  `skills/utility-claude-code-skill/references/artifact-spec-guide.md` is the artifact spec SoT either way.
