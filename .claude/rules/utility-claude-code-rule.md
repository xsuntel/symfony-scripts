...
- **Both authoring and review run through `utility-claude-code-skill`**, which dispatches on its
  `## Modes` table: the **Author** mode writes an artifact after a self-verification loop, the **Review**
  mode judges an existing artifact and writes nothing. Until 2026-09-10 the review half was the separate
  `/utility-claude-code-review` command; the criteria did not move, only the entry point —
