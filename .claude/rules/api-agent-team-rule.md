...
`utility` rules. Never fix just one. The **indented notes are deliberately team-specific** — they record what
...
- **The provider build skills are not this team's roster** — since the 2026-08-29 transfer,
  `app-src-service-api-{rest,oauth2,websocket}-build-skill` are called by `app-agent-team`. Do not call
  them yourself.
  > ✅ `[Verified]` 2026-09-09 — **those three skills exist on disk and the route is live.** The handoff
  > direction is unchanged; what changed is the receiving team's action. This note previously named five
  > `api-providers-*-build-skill` directories, said they did not exist, and told the receiving team to
  > report a provider path as **unroutable** — all of which followed from a design-era name the
  > implementation never used. `.claude/rules/api/` and `.claude/commands/api/` are still absent, but the
  > build skills define a criteria fallback, so that gap does not block routing.
...
- **`[MUST]`** — direct-spawning outside the roster; the four teams spawning one another (the single
