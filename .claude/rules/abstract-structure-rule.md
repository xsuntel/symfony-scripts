...
| `utility-agent-team-rule.md`                                             | `utility-agent-team` orchestration invariants — the closed 8-agent skill-scoped roster · delegation-not-loop-ownership · over-match suppression · Commit-runs-last sequencing |
...
| `.claude/docs/abstract-orchestrator-contract-docs.md` | The operational contract **all four** orchestrators execute — `app-agent-team`, `api-agent-team`, `tools-agent-team` and `utility-agent-team` — covering spawn contract rationale · worktree isolation and its silent-failure case · write permissions · degradation semantics · retry ownership. Carries the `abstract-` prefix because it is bound to no domain, and **pairs with four agents rather than a rule**. Holds *rationale and evidence*; the agent prompts hold the imperatives, and the authoritative agent census stays in `.claude/skills/utility-claude-code-skill/references/artifact-spec-guide.md`. (This row read "three" until 2026-09-13; `utility-agent-team` has `@see`d the file since it was created on 2026-09-11) |
| `.claude/docs/utility-agent-team-docs.md`            | Design SoT for `utility-agent-team` — team composition · why it delegates loops instead of running them · the four-domain scope boundary · trade-offs. The fourth `*-agent-team-docs.md`, created 2026-09-13 alongside its paired `rules/utility-agent-team-rule.md`; both were a tracked follow-up from 2026-09-11 |
