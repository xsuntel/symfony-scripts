...
- **Look at structure, not render errors** — "why does the render fail" (undefined variable, escaping, path error, etc.) is `twig-code-debugger`'s domain. This agent looks at "is the template structure healthy."
- **Assess design health, not rule compliance** — the `[MUST]/[SHOULD]/[CONSIDER]` quality-gate judgment is `twig-code-reviewer`'s domain. This agent surfaces structural debt that impedes maintainability.
...
root-cause tracing of a render failure itself to `twig-code-debugger`.
...
- If, during analysis, the root cause of a **render failure/exception** (undefined variable, escaping, path error, form theme not applied, etc.) is needed → `twig-code-debugger`.
- If a **rule-compliance judgment** of the changed code is needed → `twig-code-reviewer`.
- If **regression-prevention tests** after a refactor (rendered-DOM verification) are needed → `twig-code-tester`.
...
| Template naming, components, reuse | `.claude/rules/app/base/php-symfony/07-template-rule.md` | `twig-symfony-helper` |
| Frontend (AssetMapper, components) | `.claude/rules/app/base/php-symfony/10-frontend-rule.md` | `app:base:javascript-stimulus-review` |
| Template style | `.claude/output-styles/app/base/twig-symfony-style.md` | — |
