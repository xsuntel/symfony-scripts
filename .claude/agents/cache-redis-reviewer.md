...
- Upstream: `agent-team`, as a **cross-domain addition** when a change under `app/src/Service/**`, `app/src/{Entity,}Repository/**`, or `app/src/MessageQueryHandler/**` also touches caching, or `cache.yaml` / `lock.yaml` directly. You are rarely the only reviewer on a change.
