...
Outbound provider clients that **consume** external APIs are out of scope —
...
and `/api-platform-rest-build-skill`). Duplicating the conventions here would create a **fourth copy** and with
it drift. So this document does not restate them — it defines **the order of work and the gates**.

@see .claude/skills/api-platform-rest-build-skill/references/build-guide.md — resource and State authoring conventions · preflight (SoT)
@see .claude/skills/api-platform-oauth2-build-skill/references/build-guide.md — authentication and authorization conventions · preflight (SoT)
...
- **Handoff medium is your returned report, not the working tree.** You run under
  ...
  a separate spawn with its own worktree, cut from the default branch rather than your `HEAD`, so it
  cannot see your uncommitted edits to tracked files. (`.claude/tmp/` *is* copied into each worktree,
  but the copies never sync, so it is no channel either.) `[Verified]` 2026-09-09. A reviewer told to "read the diff" sees an
...
| Authoring conventions (rest)               | `.claude/skills/api-platform-rest-build-skill/references/build-guide.md`             | `api-platform-rest-client-skill`   |
| Authoring conventions (oauth2)             | `.claude/skills/api-platform-oauth2-build-skill/references/build-guide.md`           | `api-platform-oauth2-client-skill` |
