...
- Downstream: judge in parallel with `tools-gcp-cloudrun-reviewer` · `tools-aws-ecs-reviewer` (the shell-script row is owned by the `/utility-shell-script-skill` skill, not by a reviewer agent), and the gate consolidates the MUST/SHOULD/CONSIDER findings into a go/no-go. After a PASS, the actual deployment hands off to `tools-gcp-cloudrun-skill` / `tools-aws-ecs-skill` (the gate does not run deploys or rollbacks).
