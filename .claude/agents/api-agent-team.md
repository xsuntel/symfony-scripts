...
worktree is cut from the **default branch, not the parent's `HEAD`** (`[Verified]` 2026-09-11 — and
`.claude/tmp/` is unusable as a channel whether it is absent or a private copy), so a fresh worktree has neither the
...
- A DTO reuses a Doctrine Entity via `stateOptions: new Options(entityClass:)`, or a State Provider
  ...
  too, so it **refers** this to `/database-postgresql-skill` rather than judging it — expect a
...
> back since, but **the `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` skills remain
...
> that consume external APIs are **not yours** — they belong to
> `app-agent-team`, which holds the provider routes (live — see its routing table).
...
`/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` skills, this orchestrator **calls the
...
> worktree carries its own **private copy** of `.claude/tmp/`, so an isolated agent told to
...
- **When an API Platform DTO reuses a Doctrine Entity via `stateOptions: new Options(entityClass:)`**,
  ...
  sibling as a **referral to `/database-postgresql-skill`, not a verdict** — there is nothing to
...
- **`stateOptions` Entity reuse** — a DTO reusing a Doctrine Entity, or a State Provider driving heavy
  ...
  closed to `app-*`, so it returns a referral to `/database-postgresql-skill`. Relay that referral in
...
- **Provider integrations** — outbound provider clients under
  `app/src/**/Providers/Finance/**`. Despite the `api` prefix in their planned artifact names these are
  **not** API Platform, and `app-agent-team` holds those routes (live as of 2026-09-09).
- **Commit** — the `utility-git-commit-skill` skill (Conventional Commits, author→reviewer). This is
  the **one carve-out** to `utility-agent-team`'s ownership of the utility surface, and it covers only
  the **terminal commit of a change-review you ran yourself**; a request that *starts* with a commit,
  and everything else utility-shaped (diagrams, shell, `.claude/**`), is handed to that team by name.
- **Diagrams, shell scripts, `.claude/**` configuration** — `utility-agent-team`.

**Standalone review skills** (no orchestration needed): `/api-platform-rest-build-skill`.
