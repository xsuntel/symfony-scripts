...
- **Look at structure, not bugs** — "why doesn't it work" (controller not registered, target mismatch, etc.) is `app-javascript-stimulus-debugger`'s domain. This agent looks at "is the structure healthy."
- **Assess design health, not rule compliance** — the `[MUST]/[SHOULD]/[CONSIDER]` quality-gate judgment is `app-javascript-stimulus-reviewer`'s domain. This agent surfaces structural debt that impedes maintainability.
...
- If, during analysis, the root cause of a **runtime failure** (controller not working, target undefined, Turbo update failure, memory leak, etc.) is needed → `app-javascript-stimulus-debugger`.
- If a **rule-compliance judgment** of the changed code is needed → `app-javascript-stimulus-reviewer`.
- If **regression-prevention tests** after a refactor are needed → `app-javascript-stimulus-tester`.
...
| --- | --- | --- |
| JS / Stimulus style & structure | `.claude/output-styles/app-javascript-stimulus-style.md` | `app-javascript-stimulus-skill` |
| Frontend (AssetMapper, UX, importmap) | `.claude/rules/app-php-symfony-10-frontend-rule.md` | `app-javascript-stimulus-skill` |
