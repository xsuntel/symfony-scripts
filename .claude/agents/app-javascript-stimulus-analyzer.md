...
for f in $(find app/assets -name '*.js' -not -path '*/vendor/*' -not -path '*/bundles/*'); do
  echo "{\"tool_input\":{\"file_path\":\"$f\"}}" | .claude/hooks/post-tool-use/app-javascript-stimulus-guard.sh
done
```
...
- Deterministic gate: `.claude/hooks/post-tool-use/app-javascript-stimulus-guard.sh` — this domain's machine-verdict layer (evidence).
