...
- Downstream: judge in parallel with `server-nginx-reviewer` · `tools-gcp-cloudrun-reviewer` (the shell-script row is owned by the `/utility-shell-script-skill` skill, not by a reviewer agent). After a PASS (go), the actual deployment hands off to `tools-aws-ecs-skill` (the gate does not run deploys or rollbacks).
