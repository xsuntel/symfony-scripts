...
This is the English variant of the base output style, and it is self-contained. It covers general
...
Its Korean counterpart is `abstract-korean-style.md`; the two differ only in language rules and prose,
and exactly one is selected in `settings.json` (`outputStyle`).
...
- No markdown in the body, no issue number in the subject, and no `Co-authored-by` (`includeCoAuthoredBy` is `false`)
