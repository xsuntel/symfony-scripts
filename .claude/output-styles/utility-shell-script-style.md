...
| Unquoted `[[ $var == *foo* ]]` | Word-splitting risk | Always double-quote and brace: `[[ "${var}" == *foo* ]]` |
| Unbraced `"$VAR"` (including `"$REPLY"`, `"$1"`) | Violates the brace convention; fails the `require-variable-braces` gate | `"${VAR}"` — no special-parameter exemption |
| Quoting a word-splitting loop: `for x in "${LIST}"` | Collapses the list to one iteration | `for x in ${LIST}` — braced, deliberately unquoted |
