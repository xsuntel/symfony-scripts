...
- Scope this to structure — leave `[MUST]/[SHOULD]/[CONSIDER]` judgments to `app-twig-symfony-reviewer` and render failures to `app-twig-symfony-debugger`. Handoff: `analyzer → reviewer → tester`.

## Output

- ADR format (Context/Decision/Consequences) + trade-offs on three axes (scalability/maintainability/performance) + alternatives, per the abstract-english-style "Architecture Design & Analysis" section. Denote inheritance/inclusion with arrows.
