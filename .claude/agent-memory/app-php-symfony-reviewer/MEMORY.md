...
- Transport selection: `async_default` (RabbitMQ, external integration · retry · DLQ, outbound provider API calls) / `async_redis` (lightweight internal tasks) / `sync` (tests · must complete before the response).
