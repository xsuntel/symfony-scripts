...
- **Stale self-described gaps** — a docs "Current Gaps"/agent "Known gaps" section can go stale
  *mid-rollout*, describing a companion file as still broken when an earlier step of the same change
  already fixed it. `grep`/`Read` the exact site the gap names fresh — do not relay the draft's own
  framing. See [[gaps-sections-go-stale-mid-change]].
