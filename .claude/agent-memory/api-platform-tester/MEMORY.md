...
- **`api-platform-author` no longer exists** — it was renamed to `api-platform-analyzer` and converted to
  the read-only Analyze axis, and the Green hand-off it used to receive was folded in here. Never hand
  Green off to an author.
...
- Role: Test (full TDD cycle) · upstream: `api-platform-reviewer` (after `[MUST]` items are resolved) /
  `api-platform-debugger` (pinning a regression after a fix) / `agent-team` or a build skill on a
  test-first intent · downstream: `api-platform-reviewer` (quality gate); defects found →
  `api-platform-debugger`; structural debt too large for one Refactor step → `api-platform-analyzer`
