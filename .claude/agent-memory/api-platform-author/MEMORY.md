...
Out of scope: outbound provider clients that **consume** external APIs —
...
`/api-platform-rest-build-skill`). **A copy here would be the fourth and would drift.** Before working, **Read**:

- `.claude/skills/api-platform-rest-build-skill/references/build-guide.md` — `## Authoring Conventions`, `## Preflight Checks`, `## Working Principles`
- `.claude/skills/api-platform-oauth2-build-skill/references/build-guide.md` — same sections
...
```bash
echo '{"tool_input":{"file_path":"app/src/ApiResource/{path}/{file}.php"}}' | .claude/hooks/post-tool-use/app-php-lint.sh
echo '{"tool_input":{"file_path":"app/src/ApiResource/{path}/{file}.php"}}' | .claude/hooks/post-tool-use/app-php-cs-fixer.sh
cd app
...
- .claude/skills/api-platform-rest-build-skill/references/build-guide.md · skills/api-platform-oauth2-build-skill/references/build-guide.md (**authoring conventions SoT**)
