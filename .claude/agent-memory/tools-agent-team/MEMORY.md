...
- **The repository has four orchestrators** (`utility-agent-team` added 2026-09-11) — application code
  (PHP, JS, Twig) and providers belong to `app-agent-team`; the API Platform exposure layer
  (`ApiResource`, `State`, `api_platform.yaml`) to `api-agent-team`; diagrams, commit messages,
  non-deploy shell and `.claude/**` to `utility-agent-team`. Deploy-context shell stays with the gate.
...
- **`message-rabbitmq-reviewer` is not on this team** — matches none of the five prefixes. Point at
  `/message-rabbitmq-skill` instead of spawning it.
- **The shell-script agent pair exists again** (retired 2026-08-16, **restored 2026-09-11**) but is
  **not on this roster** — it matches none of the five prefixes. Route to `/utility-shell-script-skill`,
  owned by `utility-agent-team` outside a deploy context; inside one, the gate already covers it.
...
- The owner is the `tools-app-deploy-skill` gate, which runs its **own fan-out** to the nginx, GCP and
  AWS reviewers (three of this roster) plus `/utility-shell-script-skill`.
