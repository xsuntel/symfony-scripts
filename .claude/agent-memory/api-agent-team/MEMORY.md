...
- **The build commands remain SoT for the authoring conventions** — the author references
  `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` via `@see` and holds no conventions of
...
- **Non-exposure work → one `app-agent-team` delegation.** Gather **every** out-of-scope path into a
  ...
  **referral to `/database-postgresql-skill`, not a verdict** — relay it and mark that layer
...
`[Verified]` 2026-09-13 — of 38 agents, 19 set `isolation: worktree` and 19 do not. **Four of the five
roster agents are isolated** (`author`, `analyzer`, `debugger`, `tester`); only `api-platform-reviewer`
shares the working tree. A worktree is cut from the **default branch, not the parent's `HEAD`**, and its `.claude/tmp/` is unusable as a channel — absent or a private copy that never syncs back (`[Verified]` 2026-09-11; the doc claim deciding which keeps flipping, so rely on the base branch instead), so:
...
- Sibling: `app-agent-team` owns the 15 `app-*` agents, the 6 infrastructure reviewers, Deploy, Commit
  and the provider routes (**live** — the 3 transport build skills). It is both the **referral** target for independent work and
