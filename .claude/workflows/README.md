...
> Last updated: 2026-09-13 (sections 2 and 3.5 corrected — the meta and shell-script domains stopped
> self-verifying on 2026-09-11, and two entries still named `commands/` files that the 2026-09-10 fold
> retired or that never existed).
>
...
@see .claude/docs/tools-agent-team-docs.md — infrastructure · data · deployment domain design (Review-only roster)
@see .claude/docs/utility-agent-team-docs.md — utility surface design (diagrams · commits · shell · `.claude`)
...
```text
skills/                = orchestrator / entry point — dispatches a mode, calls agents, consolidates artifacts
  └─ references/       = the mode's procedure, loaded only when SKILL.md points at it
...
`references/*-guide.md` joined this picture on **2026-09-10**, when `.claude/commands/` was folded into
`.claude/skills/`: a skill's `## Modes` table names which guide to `Read`, and nothing resolves that path
automatically, so an unlinked guide is dead weight.

Three execution shapes are standard. `[Verified]` 2026-09-13 (`docs/app-agent-team-docs.md` sections 4 and 3.5)

| Shape | When | Example entry point |
| --- | --- | --- |
| **Orchestrator skill + agent pair** (author→reviewer loop, max 2 retries; 3 for code domains) | multi-step work needing gates and retries | `utility-git-commit-skill`, `utility-drawio-diagram-skill`, `utility-shell-script-skill`, `utility-claude-code-skill`, `tools-app-deploy-skill` |
| **Orchestrator skill + self-verification** (no agent pair, max 3 retries) | the provider transports, whose criteria converge on one registry-resolved document per integration | `app-src-service-api-{rest,oauth2,websocket}-build-skill` |
...
> **Corrected 2026-09-13 — the second row used to name the wrong domains and a set of files that never
> existed.** It read "meta, configuration and provider domains … `utility-claude-code-skill` +
> `/utility-claude-code-skill`, provider `*-build-skill` + the `*-build` command". Two errors: the meta
> and configuration domains **stopped self-verifying on 2026-09-11**, when the
> `utility-claude-code-*` and `utility-shell-script-*` agent pairs were created and both skills handed
> their loops over; and **the per-provider `*-build` commands were never built** — the domain shipped
> organised by transport. The provider axis is now the only self-verifying instance.
...
- **Entry point:** pick the reviewer by the changed `paths` (direct routing). For example
  ...
  > routing too — from the main session, the **Review mode of their domain skill**
  > (`/cache-redis-skill`, `/database-postgresql-skill`, `/server-nginx-skill`; GCP and AWS have no
  > review mode), or `tools-app-deploy-skill`.
  >
  > **`message-rabbitmq-reviewer` matches none of the five prefixes and is reachable only through the Review mode of `/message-rabbitmq-skill`** — reach
  > it from the main session or `/message-rabbitmq-skill`. `app-agent-team` and `api-agent-team` are
...
- **Self-gates — never read exit 0 as a pass:** the author calls the existing `PostToolUse` hooks
  ...
  present. `[Verified]`
- **Variants (skill-side self-verification, no agent pair):**
  - API Platform (resources and State / authentication and authorization) —
    ...
    `## Authoring Conventions` of their own `references/build-guide.md` and reach a verdict via the same
    guide's `## Self-Verification` (adopted 2026-08-17, max 3 retries). Both are **skills, and the
    caller must name the mode** — `/api-platform-rest-build-skill` carries three (Guide · Build ·
    Review). When a third-party verdict is wanted, spawn `api-platform-reviewer` as an additional gate.
    `[Verified]` 2026-09-13
  - Providers (REST · OAuth2 · WebSocket) — each of the three transport build skills
    (`app-src-service-api-*-build-skill`) generates, then self-verifies against the registered
    integration's rule file, or against the matching `*-client-skill` when none is registered
    (max 3 retries). `[Verified]` 2026-09-09
- **Two former variants moved to the agent pair on 2026-09-11 — do not run the self-verification path
  for either.** `[Verified]` 2026-09-13
  - Claude Code configuration artifacts — `/utility-claude-code-skill` (Author mode) calls
    `utility-claude-code-author` → `utility-claude-code-reviewer`, max 2 retries, writing to the target
    path only on a `Verdict: PASS`. Criteria stay in
    `skills/utility-claude-code-skill/references/artifact-spec-guide.md` (SoT).
  - Shell scripts (`scripts/**`) — `/utility-shell-script-skill` (Author mode) calls
    `utility-shell-script-author` → `utility-shell-script-reviewer`, max 2 retries. Criteria stay in
    `rules/utility-shell-script-rule.md` and `output-styles/utility-shell-script-style.md` (SoT), with
    the guide's `## Authoring Conventions` and `## Review Procedure` as the procedure the pair executes.
  > **Both entries previously described these as commands holding the criteria and the skill
  > self-verifying against them.** That was accurate until 2026-09-11 — and it also called the two
  > entry points "commands" after the 2026-09-10 fold. In both domains **the criteria never moved**;
  > only the owner of the verdict did. Following the retired text would run a loop with no reviewer in
  > it.
...
- **Owner since 2026-09-11: `utility-agent-team`**, the fourth orchestrator, which also owns diagrams
  (`diagram/**`), shell scripts (`scripts/**`) and `.claude/**`. It holds no criteria and runs no loop
  of its own — it delegates each job with the `Skill` tool to the skill that owns its own loop and
  retry budget. **One carve-out:** a sibling orchestrator may call `utility-git-commit-skill` directly
  to commit a change-review it ran itself, because it cannot delegate to a sibling and a team switch
  merely to commit reviewed work would strand it. The carve-out covers **Commit only**; a request that
  *starts* with a commit is `utility-agent-team`'s. `[Verified]` 2026-09-13
  @see .claude/rules/utility-agent-team-rule.md — the roster and delegation invariants (SoT)
...
> `.claude/skills/abstract-agentic-harness-skill/references/graph-engineering-guide.md` § 9 holds the
> six steps to clear **before**
