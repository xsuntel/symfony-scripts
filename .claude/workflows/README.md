...
| **Orchestrator skill + command-based self-verification** (no agent pair) | meta, configuration and provider domains where the criteria converge on a single file | `utility-claude-code-skill` + `/utility-claude-code-skill`, provider `*-build-skill` + the `*-build` command |
...
- **Variants (no agent — command-based self-verification):**
  - API Platform (resources and State / authentication and authorization) —
    ...
    `## Authoring Conventions` of the `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill`
  ...
  - Claude Code configuration artifacts — the `utility-claude-code-skill` skill drafts them and
    self-verifies against the criteria in the `/utility-claude-code-skill` skill (adopted
    2026-08-08). `[Verified]`
  ...
  - Shell scripts (`scripts/**`) — the `utility-shell-script-skill` skill drafts from the
    `## Authoring Conventions` of the `/utility-shell-script-skill` skill and self-verifies via the
