...
@see .claude/docs/agent-team-docs.md — workflow-role team design (SoT, background & rationale)
@see .claude/agents/agent-team.md — cross-domain orchestrator agent (routing counterpart of the draft)
...
>
> `.claude/workflows/` is the **only** custom, non-auto-loaded directory in this tree.
>
> **Correction (2026-08-14):** an earlier revision listed `.claude/agent-memory/` here as equally
> custom and non-auto-loaded. That was wrong. Agent memory **is** an official Claude Code feature: the
> `memory:` frontmatter key takes `user` | `project` | `local`, and `project` auto-loads
> `.claude/agent-memory/<agentType>/MEMORY.md`, where `<agentType>` is the agent's `name`. `[Verified]`
> — resolver read out of the Claude Code 2.1.220 binary. The path is **flat**; the
> `agent-memory/<domain>/<agent>/` layout this repository used until 2026-08-14 meant none of the 16
> memory files ever loaded. See `.claude/rules/utility-claude-code-rule.md`
> § *Agent Memory Layout*.
...
background's SoT is `docs/agent-team-docs.md`.
...
Two execution shapes are standard. `[Verified]` (agent-team-docs §4)

| Shape | When | Example entry point |
| --- | --- | --- |
| **Orchestrator skill** (author→reviewer verification loop, up to 2 retries) | Multi-stage / gate / retry needed | `utility-git-commit-skill`, `*-build-skill`, `utility-claude-code-skill`, `tools-app-deploy-skill` |
| **Direct routing** (select the responsible reviewer by changed `paths`) | One-off single review / debug / test | domain agents such as `app-php-symfony-reviewer` |

> The app domain also has a dedicated **orchestrator agent**, `agent-team`, that performs the
...
`docs/agent-team-docs.md` §3. `[Verified]`
...
- **Entry point:** select the responsible reviewer by changed `paths` (direct routing). e.g.
  ...
  `app/src/{Entity,Repository}/**` → Postgresql Reviewer. For app-domain changes, the `agent-team`
...
- **Entry point:** `app-php-symfony-debugger` / `app-javascript-stimulus-debugger` / `app-twig-symfony-debugger`.
...
- **Entry point:** `app-php-symfony-tester` / `app-javascript-stimulus-tester` / `app-twig-symfony-tester`.
- **Mandatory:** `rules/app-php-symfony-09-testing-rule.md` (Unit/Integration/Functional boundaries). `[Verified]`

### 3.4 Build — generate then immediately verify (author→reviewer)

- **Trigger:** request to generate API / provider / config code.
- **Entry point:** an orchestrator `*-skill` → author draft (`./.claude/tmp/*-draft.md`) → reviewer
...
- **Entry point:** `utility-git-commit-skill` → `utility-git-commit-author` → `utility-git-commit-reviewer`
...
- **Entry point:** `tools-app-deploy-skill` — **fans out** the deployment assets to the domain reviewers
  (Nginx · GCP Cloud Run · AWS ECS · Shell Script), consolidates, and judges go/no-go (PASS/BLOCK). On
  PASS, the actual deploy is handed off to `tools-gcp-cloudrun-skill`/`tools-aws-ecs-skill`. The gate does
...
`docs/agent-team-docs.md` as SoT.
...
| Repository workflow design (SoT) | `.claude/docs/agent-team-docs.md` |
