# API Platform OAuth2 Build Guide

The **Build mode** of `api-platform-oauth2-build-skill` — creates or modifies the authentication and
authorization code for the API this project exposes, then self-verifies it against the SoT rules.
`SKILL.md` dispatches here via its `## Modes` table.

**This is not judgment criteria** — `rules/api-platform-rule.md` (`## Security`) and
`rules/app-php-symfony-08-security-rule.md` are SoT. This guide holds only the authoring conventions
summary and the self-verification loop.

> Until 2026-09-10 this was the slash command `/api-platform-oauth2-build`. It now runs as the build
> mode of `/api-platform-oauth2-build-skill`; `$ARGUMENTS` still carries the implementation target.
...
This guide is **inbound only** — it protects the API we expose. Outbound OAuth2/JWT for consuming
external provider APIs is out of scope; no provider build skill exists yet (see `TODO.md`).
...
each clause — this guide does not restate the criteria.
...
@see .claude/skills/api-platform-rest-build-skill/references/review-guide.md — codebase-specific checkpoints (verification procedure)
...
- Do `app/src/ApiResource/` and `app/src/Security/` exist? With no operations to protect yet, create the
  resource first via the build mode of `/api-platform-rest-build-skill`. When adding the first Voter, also confirm the
...
2. Sweep the security items in `## Review Procedure` of `.claude/skills/api-platform-rest-build-skill/references/review-guide.md`
...
PASS verdict; this guide neither runs them automatically nor commits.
