...
| Request | Mode | Read |
| --- | --- | --- |
| Authentication wiring guidance, "how does JWT work here" | Guide | this file |
| "build/implement `<target>`" — write or modify firewall, authenticator, Voter or operation `security:` | **Build** | [`references/build-guide.md`](references/build-guide.md) |

> The Build mode was the slash command `/api-platform-oauth2-build` until 2026-09-10, when
> `.claude/commands/` was folded into `.claude/skills/`. `$ARGUMENTS` carries the implementation target.
> Reviewing existing exposure-layer code belongs to the Review mode of `api-platform-rest-build-skill`.
