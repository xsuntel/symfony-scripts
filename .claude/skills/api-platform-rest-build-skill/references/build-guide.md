# API Platform Build Guide

The **Build mode** of `api-platform-rest-build-skill` — creates or modifies API Platform resource and
State code, then self-verifies it against the SoT rules. `SKILL.md` dispatches here via its `## Modes`
table; `review-guide.md` is the sibling read-only path.

**This is not judgment criteria** — `rules/api-platform-rule.md` is SoT. This guide holds only the
authoring conventions summary and the self-verification loop.

> Until 2026-09-10 this was the slash command `/api-platform-rest-build`. It now runs as the build mode
> of `/api-platform-rest-build-skill`; `$ARGUMENTS` still carries the implementation target. Because
> that one invocation now serves both build and review, **a caller must name the mode**.
...
- Validate input with `#[Assert\...]` (per-operation differences via `validationContext` groups). Apply
  ...
  is the primary target, use `api-platform-oauth2-build-skill` instead.)
...
this guide describes the *target* layout, not existing code. Verify each item below against the real
...
2. Sweep the codebase-specific checkpoints via `## Review Procedure` in
   `.claude/skills/api-platform-rest-build-skill/references/review-guide.md` (resource declaration · serialization groups · State wiring
