# API Platform Review Guide

The **Review mode** of `api-platform-rest-build-skill` — assesses the quality of API Platform resource /
State code and returns structured improvement recommendations. `SKILL.md` dispatches here via its
`## Modes` table; `build-guide.md` is the sibling that **writes** code.
...
> Until 2026-09-10 this was the slash command `/api-platform-review`. It now runs as the review mode of
> `/api-platform-rest-build-skill`; `$ARGUMENTS` still carries the target path. Because that one
> invocation now serves both build and review, **a caller must name the mode** — see the `## Modes`
> table in `SKILL.md`.
...
> exists — the same check as `## Preflight Checks` in the sibling `build-guide.md`. With no resource code in
