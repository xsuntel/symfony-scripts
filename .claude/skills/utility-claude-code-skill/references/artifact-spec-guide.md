# Claude Code Artifact Spec Guide

The **artifact spec SoT** for this repository, and the **Review mode** of `utility-claude-code-skill`.
It assesses spec and convention compliance of Claude Code config artifacts (agents, skills, rules,
output styles, settings, `CLAUDE.md`) and returns structured improvement recommendations. `SKILL.md`
dispatches here via its `## Modes` table; the authoring workflow stays in `SKILL.md`, where
`utility-claude-code-author` reads this guide as its **writing** criteria and
`utility-claude-code-reviewer` as its **judgment** criteria.

> **Not a self-verification checklist any more `[Verified]` 2026-09-13.** This sentence read "the
> authoring workflow … reads this guide as its self-verification checklist" until today, which was
> accurate from 2026-08-08 to 2026-09-11 and stale after. On 2026-09-11 the domain was given the
> `utility-claude-code-author` / `utility-claude-code-reviewer` pair and `SKILL.md` handed the loop over
> to them. **The criteria did not move — this file is still the sole SoT — only the owner of the verdict
> did.**

**This file *is* judgment criteria** — unlike every other `references/` guide in this repository. The
**per-type frontmatter and body criteria** below have no other owner. Only the **structural** criteria
(directory taxonomy, path depth, placement, and the move/rename prohibitions) live elsewhere, in
`rules/utility-claude-code-rule.md`. Every `@see` that used to point at
`commands/utility-claude-code-review.md` points here.

> **History.** From 2026-08-08 to 2026-09-10 this was the slash command `/utility-claude-code-review`,
> deliberately split from the authoring skill so that criteria and authoring had separate owners. On
> 2026-09-10 the whole `commands/` tree was folded into `skills/` by explicit user instruction, and the
> split now lives in the `## Modes` table of `SKILL.md` rather than in a file boundary. `$ARGUMENTS`
> still carries the target path, and the Mode A / Mode B distinction below is unchanged.
>
> **Read-only constraint — carried over as prose, deliberately.** As a command this file declared
> `allowed-tools: Read, Grep, Glob, WebFetch(domain:code.claude.com),
> WebFetch(domain:platform.claude.com)`. That allowlist was **not** hoisted to the merged `SKILL.md`,
> because `utility-claude-code-skill` is an authoring skill that needs `Write`/`Edit` — setting a
> read-only allowlist there would break authoring. **When running the review mode, do not write or edit
> any file**: report findings and stop. A review that edits its own target is a `[MUST]` violation of
> the `## Refactoring Suggestions` clause at the end of this guide.
...
> body criteria** — is owned by **this guide + the official docs below**; this domain has no
...
@see .claude/skills/utility-claude-code-skill/SKILL.md — artifact authoring entry point (author → reviewer loop, max 2 REDO retries)
@see .claude/agents/utility-claude-code-author.md — draft role; reads this guide as its writing criteria
@see .claude/agents/utility-claude-code-reviewer.md — PASS/REDO verdict role; reads this guide as its judgment criteria
...
2. **Record redirects.** A page that reports a canonical URL different from the one requested is a stale
   ...
   > **Confirm a reported canonical by fetching it before you act on it.** `[Verified]` 2026-09-11, now
   > across **four** sweeps: the `hooks` and `settings-reference` pages keep self-reporting a canonical
   > that differs from the URL requested, and **the shape changes every time.** On 2026-08-30 `hooks`
   > dropped the `/en/` segment (`…/docs/hooks-reference`); on 2026-09-09 it kept `/en/` and changed the
   > slug (`…/docs/en/hooks-reference`); on **2026-09-11 it reported `…/docs/en/hooks-reference` again —
   > fetched, still HTTP 404.** `settings-reference` reported `…/docs/en/settings` on 2026-09-09 and
   > **`…/docs/en/all-settings` on 2026-09-11**, a third distinct value. **Both `@see` lines are correct
   > as written**, and have been through every sweep.
   >
   > **New on 2026-09-11 — `subagents` joined them, with the most convincing false positive yet.** It
   > self-reported `…/docs/en/agents`, which **does serve a real page** — so the 404 test passes. Fetch it
   > and it is a *different document*: "Run agents in parallel", a comparison of subagents vs agent view
   > vs teams vs workflows, which itself links to `/docs/en/sub-agents` for the frontmatter reference.
   > **A 200 is not proof of a redirect** — check that the page you get is the page you wanted.
   >
   > For `settings-reference` there is positive corroboration beyond the 404 test: the live `agent-teams`
   > page deep-links to `/docs/en/settings-reference#teammatemode` and `#cleanupperioddays`, so that page
   > demonstrably still serves. Rewriting any of these `@see` lines would have broken a live link. A
   > genuine move looks like `sub-agents` → `subagents`, verified by the new URL serving *the same page*.
...
| `agent-teams` | teammate promotion of named subagents | the `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` rule here · `.claude/settings.json` · all four orchestrator agents |
...
| Slash command *(spec retained; the tree is retired here — see `## Commands Layout — retired tree`)* | `.claude/commands/**/*.md` | `description` (optional: `argument-hint`, `allowed-tools`, `model`, and the rest of the skill frontmatter set — see the command entry in the detail rules) | instructions using the full-argument variable (`\$ARGUMENTS`) or a positional argument (`\$0` is the first, `\$1` the second) |
...
- **Agent `model`**: one of `sonnet` | `opus` | `haiku` | `fable` | `inherit`, or a full model ID (e.g. `claude-opus-5`). `tools` lists **only the least privilege** (inherits all when omitted).

  `[Verified]` 2026-09-09 — **this bullet read "Defaults to `inherit` when omitted", which is wrong.** Omitting `model` falls through a **four-step resolution order**: (1) a per-invocation `model` Claude passes, (2) the definition's `model` frontmatter (where `inherit` selects the parent's model), (3) `CLAUDE_CODE_SUBAGENT_MODEL`, (4) the main conversation's model. So an omitted `model` is **not** equivalent to `model: inherit` — an env var sits between them. `CLAUDE_CODE_SUBAGENT_MODEL_FORCE=1` overrides all of it except forks and skills set to `inherit`. Before v2.1.251 the env var came first. Neither variable is set in this repository, so every agent here resolves by its own frontmatter today.
- **Agent `tools` / `disallowedTools` — resolution order and MCP patterns.** `[Verified]` 2026-09-09: **`disallowedTools` is applied first, and `tools` is then resolved against the remainder**, so a tool named in both is denied. Both fields also accept MCP patterns (`mcp__<server>`, `mcp__<server>__*`, `mcp__*`). Do not flag an MCP pattern as an invalid tool name.
- **Agent `tools` / `disallowedTools` — two accepted shapes, and a new entry form.** `[Verified]` 2026-09-02 [WebFetch: <https://code.claude.com/docs/en/subagents>]: both are now documented as **arrays**, and `tools` additionally accepts an **`Agent(subagent-name)` allowlist entry** that restricts which subagents this agent may spawn. All 38 agents here use the comma-separated string form and all 38 resolve, so **the comma form still works and is not a finding** — but do not flag the YAML-list form or an `Agent(...)` entry as invalid either. **Do not, however, recommend `Agent(...)` as the way to express an orchestrator's closed roster.** `[Verified]` 2026-09-09 — the live page annotates its own example `tools: Agent(worker, researcher)` with **"Restrict subagent spawning (main-session agents only)"**, which corroborates the long-standing repository claim that the parenthesised type list is **silently ignored inside a sub-agent definition** and applies only to an agent arriving on the main thread via `claude --agent`. Our four orchestrators are themselves spawned as sub-agents, so an `Agent(...)` list in their frontmatter would enforce nothing — which is exactly why `agent-roster-guard.sh` enforces the roster at runtime. This clause previously called `Agent(...)` "the precise way" to express the roster; acting on that would have replaced a working hook gate with a no-op.
...
- **Agent `permissionMode`**: official (`default` | `acceptEdits` | `auto` | `dontAsk` | `bypassPermissions` | `plan` | `manual`), and a project convention as of 2026-08-25. `[Verified]` 2026-09-11 — two details worth not re-deriving: **`manual` is an alias for `default`**, so do not flag the two as different modes, and **`bypassPermissions` is only available when the main conversation is already using it**, so granting it to a subagent does not escalate a session that is not. The **16 write-path agents** — the 8 `*-author`, 4 `*-tester` and 4 `*-debugger` — set `permissionMode: acceptEdits`. The reason is that subagents inherit the parent session's permission mode and `settings.json` sets `permissions.defaultMode: "plan"`, under which an author's edits and a tester's Green phase are refused; the frontmatter override is what makes those roles executable at all. Do not flag it as an unknown key, and **do** flag its absence on a write-path agent as a `[SHOULD]`. The read-only agents (4 `*-analyzer`, 14 `*-reviewer`) deliberately omit it — they must not acquire edit acceptance — and **the four orchestrators (`app-agent-team`, `api-agent-team`, `tools-agent-team`, `utility-agent-team`) omit it** because they orchestrate rather than edit. `[Verified]` 2026-09-11 — 16 + 4 + 14 + 4 accounts for all **38** agents on disk; the count moved 32 → 33 when `tools-agent-team` was added, and 33 → 38 when `utility-agent-team` and the `utility-claude-code-*` / `utility-shell-script-*` author+reviewer pairs joined the tree.

  > **The orchestrators' omission has a cost, and it is deliberate.** All four hold `Write` and are told to persist a consolidated report under `.claude/tmp/<team>/`, but with no `permissionMode` they inherit the session's mode, and `permissions.defaultMode` is `"plan"` — a read-only mode — so that write is **refused while the session is in plan mode**. `[Verified]` 2026-08-30. The resolution taken on 2026-08-30 was **not** to grant them `acceptEdits` (that would auto-approve writes by agents that also hold unrestricted `Bash` and scope `Write` by prose alone) but to make the **returned report the required channel** and the tmp copy best-effort — nothing in this repository reads those tmp files. **Do not flag the missing `permissionMode` on an orchestrator as a `[SHOULD]`**, and **do** flag as a `[MUST]` any orchestrator that reports a tmp path as written when the write was refused, retries a refused write, or reaches for `Bash` redirection to defeat the permission mode. The earlier reading of this bullet ("12 write-path … 4 `*-author` … 10 `*-reviewer`") summed to 28 because it counted only the four code domains and silently dropped the `utility-drawio-diagram-*` and `utility-git-commit-*` pairs; the `isolation` bullet below has always accounted for all 32, so that is the arithmetic to trust. Note that all four `utility-*-reviewer` agents — `utility-claude-code-reviewer`, `utility-drawio-diagram-reviewer`, `utility-git-commit-reviewer`, `utility-shell-script-reviewer` — carry `Write` (with `disallowedTools: Edit`) yet are counted read-only: their role is to return a PASS/REDO verdict, not to author, so they stay without `acceptEdits`.
- **Agent `effort` / `color` / `background`**: all official. `effort` is one of `low` | `medium` | `high` | `xhigh` | `max` (available levels depend on the model); `color` is one of `red` | `blue` | `green` | `yellow` | `purple` | `orange` | `pink` | `cyan`; `background: true` keeps the subagent in the background even when Claude asks for the foreground. `[Verified]` 2026-09-13 — **exactly two agents set any of the three**: `tools-agent-team` carries `color: orange` and `utility-agent-team` carries `color: purple`; no agent sets `effort` or `background`. Do not flag their absence, do not flag them as unknown keys if one is added, and **do not flag the two `color` values as an inconsistency** — a display colour on the orchestrators and not their specialists is cosmetic, not a convention breach. (This bullet read "No agent here sets any of the three" until 2026-08-30, when `tools-agent-team` was created, then "exactly one agent" until 2026-09-13, when the same omission repeated for `utility-agent-team` — added 2026-09-11 and uncounted for two days. The census is drifting once per orchestrator; check `grep -l '^color:' .claude/agents/*.md` rather than trusting this sentence.)
- **Agent `isolation` / `maxTurns`**: both official and both project conventions — every agent here sets `maxTurns` (30, **45 for the 4 `*-tester`** whose Red-Green-Refactor cycle runs six or more gate commands per invocation, 50 for each of `app-agent-team` and `api-agent-team`, and **40 for each of `tools-agent-team` and `utility-agent-team`** — `tools-agent-team`'s single Review axis runs one fan-out plus consolidation with no author→reviewer loop to fund, and `utility-agent-team` delegates every job to a skill that owns its own loop and retry budget, so do not flag either lower value as a deviation), and the 19 domain agents scoped to `app/**` sources set `isolation: worktree` (the 15 `app-*` agents plus `api-platform-analyzer`, `api-platform-author`, `api-platform-debugger`, `api-platform-tester`). The convention follows the **agent's domain scope, not whether it writes**: `app-php-symfony-analyzer` and `api-platform-analyzer` are read-only (`tools: Read, Grep, Glob, Bash`, `disallowedTools: Edit, Write`) yet still carry `isolation: worktree`, so do not flag a read-only agent's worktree isolation as inconsistent. Do not flag either key as unknown. The omissions are deliberate: **the four orchestrators (`app-agent-team`, `api-agent-team`, `tools-agent-team`, `utility-agent-team`) orchestrate rather than edit**, `api-platform-reviewer` and the six infrastructure reviewers (`cache-redis-reviewer`, `database-postgresql-reviewer`, `message-rabbitmq-reviewer`, `server-nginx-reviewer`, `tools-aws-ecs-reviewer`, `tools-gcp-cloudrun-reviewer`) are read-only reviewers outside that scope, and the `utility-git-commit-*` pair **must** see the real working tree (`git diff --cached`) — flag an added `isolation: worktree` on those two as a `[MUST]`. `api-platform-reviewer` is the same case for a different reason: it is the only non-isolated member of its own domain roster, which is what lets it read a tmp artifact the other four cannot — flag an added `isolation: worktree` there as a `[MUST]` too. The `utility-drawio-diagram-author` / `utility-drawio-diagram-reviewer` pair is likewise omitted by design: the author reads real `diagram/**` files to learn existing conventions, and both write only under `.claude/tmp/`. The remaining two `utility-*` pairs are omitted for the same reason: `utility-claude-code-author` / `utility-claude-code-reviewer` must read the live `.claude/**` tree they are judging, and `utility-shell-script-author` / `utility-shell-script-reviewer` must read the real `scripts/**` and `.claude/hooks/**` — a worktree cut from the default branch would hide the very edits under review. Those **19** omissions plus the 19 above account for all **38** agents. `[Verified]` 2026-09-11 — the count moved 31 → 32 when `agent-team` was split into `app-agent-team` and `api-agent-team`, then 32 → 33 when `tools-agent-team` was added, then 33 → 38 with `utility-agent-team` and the two new `utility-*` author+reviewer pairs; the **specialist** count is now 34.
- **Consequence of `isolation: worktree` — do not "fix" the handoff back to `git diff`.** `[Verified]` 2026-09-11 [WebFetch: <https://code.claude.com/docs/en/subagents>]: the worktree is branched **by default from the repository's default branch rather than the parent session's `HEAD`**. So an isolated agent sees **none of a sibling agent's uncommitted edits to tracked files**, and a tmp path is never a channel between two agents.

  > ⚠️ **The untracked/gitignored half of this claim has flipped once and flipped back — treat it as volatile and never build on it.** Four sweeps: on 2026-08-30 the page said **tracked content only**; on 2026-09-09 it said **"Includes untracked and gitignored files from base branch"**, and 27 files here were rewritten to match; on **2026-09-11** it read "Includes tracked files only (no untracked or gitignored files)" — back to the original; on **2026-09-13 it reads "Includes tracked files and respects `.gitignore`"**, a fourth phrasing that agrees with the 2026-08-30 and 2026-09-11 readings. Three of four sweeps say tracked-only, so that is the likelier truth — but whether the 2026-09-09 divergence was a doc revision or a rendering artifact of the fetch, the lesson is unchanged: **this field is not sound footing for any instruction.**
  >
  > **What is stable, and what every conclusion here now rests on, is the base branch.** That has read the same across all three sweeps: the worktree is cut from the *default branch*, not the parent's `HEAD`. That alone is sufficient for both operative verdicts — the inline-diff handoff is mandatory (a sibling's uncommitted edits to tracked files are invisible either way), and `.claude/tmp/` is not a handoff channel (absent under one reading, a divergent private copy under the other, useless under both).
  >
  > **So write instructions that hold under either reading.** Never assert to an agent that a gitignored path such as `app/vendor` either *is* or *is not* present in its worktree — tell it to check and install if the gate matters. Flag as a `[MUST]` any edit that reintroduces a dependency on the untracked/gitignored half in either direction.
...
- **Skill prefix — `abstract-` is permitted and is not a taxonomy violation.** `[Verified]` 2026-09-09: one skill carries it — `abstract-agentic-harness-skill` (created 2026-09-06 as three `abstract-*-engineering-skill` directories, merged into one on 2026-09-09) — on exactly the footing that lets `abstract-structure-rule.md` and `abstract-*-style.md` skip a domain prefix: the skill is bound to no domain. **Do not flag the prefix**, and do not flag the missing `rule + docs + reviewer-agent` quartet: it is a design skill that holds no criteria and references the existing SoTs by `@see`, so a quartet member would duplicate an owned criterion. **Do** flag a missing `-skill` suffix, a `name` diverging from the directory, or an `abstract-` prefix on a skill that plainly belongs to one domain.
...
- **Command frontmatter — commands are skills now, with two exceptions.** `[Verified]` 2026-09-02 [WebFetch: <https://code.claude.com/docs/en/skills>]: "Custom commands have been merged into skills. A file at `.claude/commands/deploy.md` and a skill at `.claude/skills/deploy/SKILL.md` both create `/deploy` and work the same way." A command file accepts nearly the full skill frontmatter set — beyond `description` / `argument-hint` / `allowed-tools` / `model`, that includes `when_to_use`, `disable-model-invocation`, `user-invocable`, `disallowed-tools`, `effort`, `context` (`fork`), `agent`, `background`, `hooks`, `shell`, `metadata`. Do **not** flag any of these as an unknown key on a command or a skill.
  ...
  **Three more optional keys, added to the live page by 2026-09-13** `[Verified]` 2026-09-13 [WebFetch: <https://code.claude.com/docs/en/skills>]: **`arguments`** (a space-separated string or YAML list, naming the arguments the skill takes — distinct from the free-text `argument-hint`), **`license`** (string) and **`compatibility`** (string, **≤ 500 characters**). Do not flag any of the three as an unknown key. None is set by any skill in this repository, so do not flag their absence either. Note also that **every skill field is officially optional** and boolean fields accept `yes`/`no`/`on`/`off`/`1`/`0`/`true`/`false` in any case — a `disable-model-invocation: yes` is valid, not a type error.

  > **This is the drift class the `## Official Documentation Currency` section calls the most damaging, caught in the act.** A new optional key that this guide does not list produces a **confident false positive**: a perfectly valid skill gets a `[MUST]` for an "unknown key". Re-read the live field table on every sweep rather than diffing only the keys already named here.
...
- **`settings.json` — `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` must stay `"0"`.** `[Verified]` 2026-09-02 [WebFetch: <https://code.claude.com/docs/en/agent-teams>]. The premise still holds — "while agent teams are enabled, a subagent that Claude **names** launches as a teammate, so teams can form even when you didn't ask for one" — but **the stated reason has been corrected**, see the box below. The operative reasons are now:
  - **"No nested teams: teammates cannot spawn their own teammates."** This is decisive here: all four orchestrators (`app-agent-team`, `api-agent-team`, `tools-agent-team`, `utility-agent-team`) exist to spawn other agents or delegate to skills, and `api-agent-team` additionally spawns `app-agent-team` as its one cross-boundary delegation (Mode D, capped at one per invocation — the documented exception in `rules/api-agent-team-rule.md`). Promoted to teammates, that whole routing layer stops working.

    > **New on 2026-09-13 — promotion has two documented exemptions, and neither rescues us.** `[Verified]` [WebFetch: <https://code.claude.com/docs/en/agent-teams>]: Claude launches a teammate when it calls the Agent tool with a `name` while teams are enabled, "**unless the call is a fork or passes `isolation` on the call itself**". That is `isolation` **passed on the call**, not the frontmatter key — so the 19 agents here carrying `isolation: worktree` in their definitions are **not** covered, and the four orchestrators set no `isolation` at all. Do not read this exemption as a reason to relax the `"0"`.
