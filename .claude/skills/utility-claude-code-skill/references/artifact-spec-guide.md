...
dispatches here via its `## Modes` table; the authoring workflow stays in `SKILL.md` and reads this
guide as its self-verification checklist.
...
- **Consequence of `isolation: worktree` — do not "fix" the handoff back to `git diff`.** `[Verified]` 2026-09-11 [WebFetch: <https://code.claude.com/docs/en/subagents>]: the worktree is branched **by default from the repository's default branch rather than the parent session's `HEAD`**. So an isolated agent sees **none of a sibling agent's uncommitted edits to tracked files**, and a tmp path is never a channel between two agents.
  ...
  > ⚠️ **The untracked/gitignored half of this claim has now flipped twice — treat it as volatile and never build on it.** Three sweeps, three readings of the same sentence: on 2026-08-30 the page said **tracked content only**; on 2026-09-09 it said **"Includes untracked and gitignored files from base branch"**, and 27 files here were rewritten to match; on **2026-09-11 it reads "Includes tracked files only (no untracked or gitignored files)"** — back to the original. Whether the flip is a doc revision or a rendering artifact of the fetch, the lesson is the same: **this field is not sound footing for any instruction.**
  >
