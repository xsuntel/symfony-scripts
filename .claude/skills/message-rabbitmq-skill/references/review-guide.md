# RabbitMQ / Messenger Review Guide

The **Review mode** of `message-rabbitmq-skill` — assesses the quality of Symfony Messenger / RabbitMQ
usage code and returns structured improvement recommendations. `SKILL.md` dispatches here via its
`## Modes` table.

**This is not judgment criteria** — `rules/message-rabbitmq-rule.md` is SoT. This guide holds only the
target selection, the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/message-rabbitmq-review`. It now runs as the review mode
> of `/message-rabbitmq-skill`; `$ARGUMENTS` still carries the target path.
