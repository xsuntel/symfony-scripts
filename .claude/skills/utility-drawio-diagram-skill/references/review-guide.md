# draw.io Diagram Review Guide

The **Review mode** of `utility-drawio-diagram-skill` — assesses the structural integrity and
project-convention compliance of an existing `.drawio` file. `SKILL.md` dispatches here via its
`## Modes` table; the author → reviewer authoring loop stays in `SKILL.md`.

**This is not judgment criteria** — `rules/utility-drawio-diagram-rule.md` (structure, storage format,
canvas, palette) and `output-styles/utility-drawio-diagram-style.md` (XML authoring style) are SoT.
This guide holds only the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/utility-drawio-diagram-review`. It now runs as the
> review mode of `/utility-drawio-diagram-skill`; `$ARGUMENTS` still carries the target path. **The
> mode boundary that used to be a file boundary now lives in the `## Modes` table** — this guide is
> the read-only path and never writes to `diagram/**`.
...
> This guide is **for reviewing an existing file only**. Authoring and modification run through the
> **Author mode** in `SKILL.md`, which drives the `utility-drawio-diagram-author` →
> `utility-drawio-diagram-reviewer` loop; the generation contract is owned by the author agent.
