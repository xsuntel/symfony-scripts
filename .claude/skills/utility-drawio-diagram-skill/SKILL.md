...
description: "Authors, reviews, and applies draw.io diagrams (.drawio) under diagram/**. Runs in two modes: Author (a two-agent author/reviewer loop that writes the target file on a PASS verdict) and Review (judges an existing .drawio for structural integrity and convention compliance without writing). Always use it for natural-language requests like 'draw a diagram', 'make me a drawio', 'edit this .drawio', 'add an architecture diagram', or 'review this .drawio' (in any language)."
argument-hint: "[review: path of the .drawio file to judge — omit to author]"
---
...
| Request | Mode | Read |
| --- | --- | --- |
| "draw / create / edit a diagram" — produce or change a `.drawio` | **Author** | this file — the `## Workflow` below |
| "review `<path>`" — judge an existing file, write nothing | **Review** | [`references/review-guide.md`](references/review-guide.md) |

> The Review mode was the slash command `/utility-drawio-diagram-review` until 2026-09-10, when
> `.claude/commands/` was folded into `.claude/skills/`. `$ARGUMENTS` carries the target path.
> **Do not run the two-agent loop for a Review request** — the loop exists to write, and Review does
> not write to `diagram/**` at all.
...
@see .claude/skills/utility-drawio-diagram-skill/references/review-guide.md — standalone review of an existing file (the path that does not use this loop)
