# Redis / Cache Review Guide

The **Review mode** of `cache-redis-skill` — assesses the quality of Symfony Cache / Redis usage code
and returns structured improvement recommendations. `SKILL.md` dispatches here via its `## Modes` table.

**This is not judgment criteria** — `rules/cache-redis-rule.md` is SoT. This guide holds only the target
selection, the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/cache-redis-review`. It now runs as the review mode of
> `/cache-redis-skill`; `$ARGUMENTS` still carries the target path.
...
and provide concrete fixes (improved code snippets). Do not restate the criteria in this guide.
