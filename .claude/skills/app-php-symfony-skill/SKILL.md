...
description: Use in this PHP 8.4 / Symfony 8 project to analyze the codebase structure, guide the use of external libraries, or review code changes. Covers the Symfony app layout, the multi-EntityManager topology, Messenger message flow, Composer/importmap package management, and the PR review checklist. Triggered by architecture documentation, domain-boundary mapping, dependency mapping, composer require, importmap:require, library compatibility checks, code review, bug identification, and Pull Request improvement requests. Also assesses the quality of one named PHP file and returns structured improvement recommendations — say "review <path>" to run that mode.
argument-hint: '[path to the PHP file to review — omit for guidance or a changeset review]'
---
...
| Reviewing a **changeset** (`git diff`), identifying bugs, PR improvement suggestions | Part 3 — Code Review |
| Reviewing **one named file** — "review `<path>`" | [`references/review-guide.md`](references/review-guide.md) |

> The last row was the slash command `/app-php-symfony-review` until 2026-09-10, when
> `.claude/commands/` was folded into `.claude/skills/`. `$ARGUMENTS` carries the target path. Keep the
> two review paths distinct: **Part 3 judges a diff across layers, the guide judges one file in depth.**
