# PHP / Symfony Review Guide

The **Review mode** of `app-php-symfony-skill` — assesses the quality of a PHP file and returns
structured improvement recommendations. `SKILL.md` dispatches here via its `## Modes` table; Part 1
(codebase analysis) and Part 2 (library usage) stay in `SKILL.md`.
...
> Until 2026-09-10 this was the slash command `/app-php-symfony-review`. It now runs as the review mode
> of `/app-php-symfony-skill`; `$ARGUMENTS` still carries the target path.
