---
name: api-platform-oauth2-build
...
# API Platform OAuth2/JWT Build Helper (authentication server configuration)
...
token and calls the API is out of scope (→ `api-platform-oauth2-client`).
...
@see .claude/rules/api/base/api-platform-rule.md — `## Security` section (operation security·stateless·denial messages, SoT)
@see .claude/rules/app/base/php-symfony/08-security-rule.md — security & rate limiting (SoT)
...
@see .claude/skills/api/base/api-platform-oauth2-client/SKILL.md — client flow that consumes the API with an issued token
