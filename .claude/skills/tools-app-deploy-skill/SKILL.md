...
`tools-aws-ecs-skill` (AWS). To review a single domain only, invoke its Review mode directly
(`/server-nginx-skill`, `/utility-shell-script-skill`).
...
| `scripts/**/*.sh`, `scripts/**/entrypoint.sh` | `/utility-shell-script-skill` Review mode (skill — no agent) | `utility-shell-script-rule.md` |

> **Two dispatch shapes in this table.** The nginx / GCP / AWS rows name **reviewer agents**, spawned in
> parallel. The shell row names a **skill Review mode**, which runs in this session instead.
>
> **Keep the shell row on the skill mode, and note the reason changed on 2026-09-11.** It used to be
> that no shell agent existed at all — the `utility-shell-script-author` / `-reviewer` pair was retired
> on 2026-08-16. That pair has since been **restored**, so the old rationale is void, but the row is
> still correct: `utility-shell-script-reviewer` gates on a **tmp draft and its sidecar** and judges
> PASS/REDO inside the authoring loop. It cannot review an arbitrary existing `.sh` — that is Mode A of
> the skill's Review mode, which is exactly what this gate needs. Spawning the agent here would hit its
> freshness gate and return `stale-or-missing-draft`.
>
> Keep the nginx row on its agent even though `/server-nginx-skill` now has a Review mode of its own.
> The shapes are not interchangeable: an agent spawn is parallel and returns a report, a skill mode runs
> inline and costs turns in this session. Swapping the three agent rows to skill modes would serialise
> the whole fan-out. (The shell row named a slash command until 2026-09-10, when `.claude/commands/`
> folded into `.claude/skills/`; only its name changed, not its dispatch shape.)
>
...
3. **Fan out to reviewers**
   ...
   - For the shell row, run the Review mode of `/utility-shell-script-skill` per changed script instead — it is a skill, not an agent, so it runs in this session rather than as a parallel spawn.
