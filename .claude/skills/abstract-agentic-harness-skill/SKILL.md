...
abstract-agentic-harness-skill  (this skill. It never writes files itself)
        ...
        ↓
utility-claude-code-skill      = draft a single artifact → review → record at the target path
        │ runs it as a two-agent loop (max 2 REDO retries, writes only on Verdict: PASS)
        ↓
utility-claude-code-author  →  utility-claude-code-reviewer
        │ both read the same criteria from
        ↓
