...
@see .claude/docs/abstract-orchestrator-contract-docs.md — the operating contract shared by the four teams (rationale)
...
**Create all three slugs plus the memory in one change.** The teams have been symmetric on this
composition since 2026-08-30; leaving one out makes that team shaped differently from the others and
impossible to compare against them. `utility-agent-team` is the cautionary case — added 2026-09-11 with
only the agent and its memory, it went two days without a rule or docs file, and three separate sites
grew workarounds pointing at the agent prompt in their place before the pair was completed on
2026-09-13.
...
- **`maxTurns` is decided by roster size** — `tools-agent-team` has only 5 members and a single Review
  axis with no author→reviewer loop to fund, so it gets 40; `utility-agent-team` also gets 40 because it
  funds no loop at all, delegating every one to the skill that owns it. Do not copy from the axis.
- **`Skill` must be in `tools`** — every team has a path that delegates to a skill, and for
  `utility-agent-team` delegation is the *only* path.
