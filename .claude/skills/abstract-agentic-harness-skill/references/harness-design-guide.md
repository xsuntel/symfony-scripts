...
| the account and trade-offs of a role-axis, routing or merge decision | the relevant section of `docs/{app,api,tools,utility}-agent-team-docs.md` — all four resolve as of 2026-09-13, when `utility-agent-team` gained both `docs/utility-agent-team-docs.md` and `rules/utility-agent-team-rule.md`. The glob read `{app,api,tools}` from 2026-09-11 to 2026-09-13 precisely because a pointer to a file that is not there fails silently at read time; widen a glob only after the target exists |
