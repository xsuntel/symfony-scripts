...
| Reviewing a **changeset** (`git diff`), identifying bugs, PR suggestions | Part 3 — Code Review         |
| Reviewing **one named file** — "review `<path>`"                        | [`references/review-guide.md`](references/review-guide.md) |

> The last row was the slash command `/app-javascript-stimulus-review` until 2026-09-10, when
> `.claude/commands/` was folded into `.claude/skills/`. `$ARGUMENTS` carries the target path. Keep the
> two review paths distinct: **Part 3 judges a diff, the guide judges one file in depth.**
