# JavaScript / Stimulus Review Guide

The **Review mode** of `app-javascript-stimulus-skill` — assesses the quality of a JavaScript file and
returns structured improvement recommendations. `SKILL.md` dispatches here via its `## Modes` table.

**This is not judgment criteria** — the rule files and the output style below are SoT. This guide holds
only the target selection, the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/app-javascript-stimulus-review`. It now runs as the
> review mode of `/app-javascript-stimulus-skill`; `$ARGUMENTS` still carries the target path.
