# PostgreSQL / Doctrine Review Guide

The **Review mode** of `database-postgresql-skill` — assesses the quality of Doctrine Entity /
Repository / Migration code and returns structured improvement recommendations. `SKILL.md` dispatches
here via its `## Modes` table.

**This is not judgment criteria** — `rules/database-postgresql-rule.md` is SoT. This guide holds only
the target selection, the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/database-postgresql-review`. It now runs as the review
> mode of `/database-postgresql-skill`; `$ARGUMENTS` still carries the target path.
...
and provide concrete fixes (improved code snippets). Do not restate the criteria in this guide — when
