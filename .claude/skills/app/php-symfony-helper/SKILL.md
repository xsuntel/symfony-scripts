...
@see .claude/rules/app/php-symfony/00-overview-rule.md ~ 11-performance-rule.md — full PHP/Symfony standards (SoT)
...
3. **EntityManager topology** → read `app/config/packages/doctrine.yaml` to confirm the EM ↔ DB ↔ domain mapping (SoT is the rule `database/postgresql-rule.md ## Multiple EntityManagers` table). No cross-manager associations.
...
|---|---|
| `guzzlehttp/guzzle`, cURL, `file_get_contents` | `HttpClientInterface` (`rules/api/rest-rule.md`) |
...
| Doctrine / DB | `rules/database/postgresql-rule.md`, `05-doctrine-rule.md` |
| External API | `rules/api/rest-rule.md` + per-provider rules |
