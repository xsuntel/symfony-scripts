...
> modes below are **A — existing file review** and **B — draft verification**; the criteria in them are
> unchanged.
>
> ⚠️ **Mode B is no longer *self*-verification `[Verified]` 2026-09-13.** This line called it that until
> today, contradicting the history note further down this same file: on 2026-09-11 the
> `utility-shell-script-author` / `utility-shell-script-reviewer` pair was restored and the skill handed
> the loop back to it, so Mode B is what **`utility-shell-script-reviewer`** executes against a draft —
> not something the skill runs on its own output. A reader who stopped at this line would run a loop
> with no reviewer in it.
