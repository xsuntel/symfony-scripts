...
@see .claude/commands/utility/claude/code-config-review.md — per-type frontmatter rules & verification checklist (SoT)
...
4. **Self-verify before writing**
   - This domain has **no author/reviewer sub-agent pair** (unlike `utility/shell-script` and `utility/git`); the skill verifies its own draft — there is no downstream reviewer to defer to.
...
5. **Write to the target path**
   ...
   - The target path must follow the existing `<kind>/<domain>/<tier>/…` taxonomy — mirror the closest sibling of the same kind. **Never** flatten the taxonomy, and never move or rename an existing file (no `mv`, and no write-to-new-path + delete-old-path).
