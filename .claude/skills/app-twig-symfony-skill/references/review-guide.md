# Twig / Symfony Review Guide

The **Review mode** of `app-twig-symfony-skill` — assesses the quality of a Twig template and returns
structured improvement recommendations. `SKILL.md` dispatches here via its `## Modes` table.

**This is not judgment criteria** — the rule files below are SoT. This guide holds only the target
selection, the review procedure and the output format.

> Until 2026-09-10 this was the slash command `/app-twig-symfony-review`. It now runs as the review mode
> of `/app-twig-symfony-skill`; `$ARGUMENTS` still carries the target path.
...
The review methodology, cross-checks and output format are below; `SKILL.md` holds the template-structure
analysis and library-usage guidance.
