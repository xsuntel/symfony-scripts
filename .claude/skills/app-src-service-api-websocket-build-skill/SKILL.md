---
name: app-src-service-api-websocket-build-skill
description: 'Implements or modifies WebSocket integration code for a third-party API under app/src/Service/** through a generate → self-verify loop. Covers realtime streams, subscription management and reconnection. Use it for requests like ''write the WebSocket integration'', ''implement the realtime feed'', ''fix the subscription'' (in any language). The target integration is fixed in step 1 from references/provider-registry.md or from the user''s instruction; the criteria come from that integration''s rule file when registered, and otherwise from app-src-service-api-websocket-client-skill. For guidance without generating code use app-src-service-api-websocket-client-skill; for authentication use app-src-service-api-oauth2-build-skill.'
---

# WebSocket Build Skill (third-party API integration)

Implements WebSocket integration code under `app/src/Service/**` through a **generate → self-verify**
...
- Verification output: `./.claude/tmp/app/service/{integration}-api-websocket-review.md` (gitignored)

@see .claude/skills/app-src-service-api-websocket-build-skill/references/provider-registry.md — integration → code path · criteria · commands · tmp path · secrets
@see .claude/skills/app-src-service-api-websocket-client-skill/SKILL.md — the transport-layer criteria and checklist (the fallback criteria)
...
1. **Precondition — fix the target integration and its criteria**
   ...
   - **Resolve the criteria in this order, and state which one you landed on:**
     ...
     2. otherwise **`app-src-service-api-websocket-client-skill`**, whose connection-lifetime
        conventions and checklist serve as the criteria, together with `message-rabbitmq-rule.md` and
        the `app-php-symfony-*` rules.
   - Falling back is the normal path for an unregistered integration — **it is not a blocker.** Report
     that no integration-specific SoT exists yet, and that channel identifiers and the
     concurrent-subscription limit therefore **cannot be verified** and must be confirmed against the
     vendor's official documentation.
   - Confirm the implementation target (channel, subscription identifier, capability, scope of change)
     is specified. If it is unclear, ask once and stop — **never widen the scope on your own.**
...
4. **Branch on the verdict**
   - **PASS:** report the change summary, point at the quality gates below, and stop (never commit
     ...
     >
     > A static gate passing is **not** evidence a stream works — reconnection, subscription restore and
     > keep-alive are only observable at runtime. Report them as unverified unless a worker was actually
     > run.
...
- Never include a secret value (the keys, tokens and approval keys named in the registry's `Secrets`
  column) in any output, and never log a handshake URL verbatim.
