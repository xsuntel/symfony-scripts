...
@see .claude/skills/api-platform-rest-build-skill/references/build-guide.md — authoring conventions (SoT)
@see .claude/skills/api-platform-oauth2-build-skill/references/build-guide.md — authentication/authorization authoring conventions (SoT)
...
consume external provider APIs are a different concern entirely — despite the
...
- **The criteria live in three places and no more.** `rules/api-platform-rule.md` is the verdict SoT;
  `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` are the SoT for the **authoring
  conventions**; `/api-platform-rest-build-skill` is the standalone review entry point. Every agent in this
...
- **Four of the five are `isolation: worktree`.** A worktree is cut from the **default branch, not the parent's `HEAD`** (`[Verified]` 2026-09-11 — whether `.claude/tmp/` is absent or present-but-private depends on a doc claim that keeps flipping; it is unusable as a channel either way),
...
| DTO `stateOptions` (Entity reuse), N+1, migrations | **`app-agent-team`** (delegated) — but its roster closed to `app-*` on 2026-08-29, so it **returns a referral to `/database-postgresql-skill`, not a verdict**. Relay it and mark the layer unreviewed | `database-postgresql-rule.md` |
...
>   so the `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` skills were created as a
...
>   `/api-platform-rest-build-skill` skill survived, verification references those two and the rule (SoT) via `@see`.
...
>   commands, `api-platform-reviewer`, `/api-platform-rest-build-skill`), so a fourth copy would guarantee
...
| # | Finding | Status |
| --- | --- | --- |
| 1 | A DTO reusing an Entity via `stateOptions` matches both `api-platform-rule.md` and `database-postgresql-rule.md` | ⚠️ **Live, and widened 2026-08-29** — `app-agent-team`'s roster closed to `app-*` hours after revision 5, so **neither orchestrator can spawn `database-postgresql-reviewer`**. There is no second verdict to merge: the delegation returns a **referral** to `/database-postgresql-skill` that a human closes out. Relay it and mark the persistence layer unreviewed — never report the merge as done. See umbrella §5.3 |
...
| Authoring SoT | `.claude/skills/api-platform-rest-build-skill/references/build-guide.md` · `skills/api-platform-oauth2-build-skill/references/build-guide.md` | authoring conventions (no agent duplicates these) |
| Review entry point | `.claude/skills/api-platform-rest-build-skill/references/review-guide.md` | `/api-platform-rest-build-skill` standalone verdict |
