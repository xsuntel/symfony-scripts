...
- **Spec, naming, tooling and convention SoT** → the body of `.claude/skills/utility-claude-code-skill/references/artifact-spec-guide.md`

@see .claude/rules/utility-claude-code-rule.md — directory structure & placement criteria (SoT)
@see .claude/skills/utility-claude-code-skill/references/artifact-spec-guide.md — per-type spec, naming, tooling & convention verdicts (SoT)
...
repository's four orchestrators write bare `Agent` without parentheses.
...
**Slash command** (`.claude/commands/**/*.md` — **spec retained, tree retired 2026-09-10**)
...
took a single argument, so they standardised on `$ARGUMENTS` — and the 13 `references/*-guide.md` files they became still do, because a `SKILL.md` accepts `argument-hint` and the same substitution engine. `[Verified]` 2026-09-10
...
- The taxonomy is encoded as a hyphenated prefix rather than directories:
  ...
  `app-src-service-api-rest-client-skill`, `cache-redis-skill`.
- The `-skill` suffix is mandatory (a project convention; neither the standard nor the harness requires
  ...
  collision guard**: `.claude/skills/<name>/`, `.claude/workflows/<name>.js` and the built-ins share one
  `/` namespace, so a name clash silently masks one of them. (`.claude/commands/<name>.md` shared it too,
  until that tree was retired on 2026-09-10 — the guard matters more since, not less, because the folded
  procedures no longer have their own `-review` / `-build` names to be distinguished by.)
...
| Stripping the domain prefix from a slug (`skills/php-symfony-skill/`) | Keep the domain prefix (`skills/app-php-symfony-skill/`) |
...
`/utility-claude-code-skill` skill to review an existing file's spec and convention compliance.
