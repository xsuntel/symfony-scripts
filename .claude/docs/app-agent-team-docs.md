...
skills/<n>/references/ = holders of procedure. Reached only via the SKILL.md `## Modes` table, as a skill's
           second mode or its self-verification basis. (commands/ was retired 2026-09-10 and is empty.)
```
...
The current **34 specialist** agents, organised by **workflow role (rows) × technical domain (columns)**.
`[Verified]` 2026-09-11 — **38 files** in the `agents/` tree, less the **four orchestrators**
(`app-agent-team.md`, `api-agent-team.md`, `tools-agent-team.md`, `utility-agent-team.md`). The
orchestrator count has moved 1 → 2 → 3 → 4; the specialist count moved 30 → 34 when the
`utility-claude-code-*` and `utility-shell-script-*` author+reviewer pairs were added.
...
| **Build** (generate-verify) | `app-php-symfony-author` | `app-javascript-stimulus-author` · `app-twig-symfony-author` | — | `api-platform-author` (+ the 3 provider transport build skills self-verify against the registered integration's rule, or its `*-client-skill` when none is registered) | `utility-shell-script-author` + `utility-shell-script-reviewer` · `utility-claude-code-author` + `utility-claude-code-reviewer` |
...
| **Deploy** | — | — | (deploy gate: reuses `server-nginx-reviewer`, `tools-gcp-cloudrun-reviewer`, `tools-aws-ecs-reviewer` + the `/utility-shell-script-skill` skill) | — | — |
...
> (section 4).
>
> **Claude Code configuration artifacts left the Build team on 2026-08-08 and rejoined it on
> 2026-09-11.** The author and reviewer agents were first merged into the review command, leaving the
> skill to self-verify against it; on 2026-09-11 the
> `utility-claude-code-author` / `utility-claude-code-reviewer` pair was created and the skill handed the
> loop back. The same round trip happened to **Shell Script** — its pair was retired on 2026-08-16 and
> restored on 2026-09-11. Both domains are ordinary Build members again; only
> `skills/utility-claude-code-skill/references/artifact-spec-guide.md` and
> `rules/utility-shell-script-rule.md` remain SoT, exactly as before (section 3.5).
>
> **Providers are the one Build axis still agentless** — the 10 author and reviewer agents were merged
> away on 2026-08-15 and never rebuilt. What exists today is **three transport build skills**
> (`app-src-service-api-{rest,oauth2,websocket}-build-skill`) running a generate → self-verify loop, not
> the 5 per-provider `*-build` commands the original plan named; those commands were never built. See
> the ⚠️ note in `rules/abstract-structure-rule.md` (section 3.5).
>
> **API Platform stopped being a Build team member on 2026-08-17** — `api-platform-author` was merged
> into the `/api-platform-rest-build-skill` and `/api-platform-oauth2-build-skill` skills and the agent itself
...
> merged into the slash command `/utility-shell-script-skill`, and the `utility-shell-script-skill`
...
| `scripts/**/*.sh`, `scripts/**/entrypoint.sh` | (no agent — the `/utility-shell-script-skill` skill) | `utility-shell-script-rule.md` |
...
- **The app-domain authors' self-gates reuse the existing `PostToolUse` hooks via the CLI.** Unlike the
  ...
  **take no `$1` argument and read only stdin JSON.** The author therefore feeds them the payload
  ...
  echo '{"tool_input":{"file_path":"app/assets/controllers/x_controller.js"}}' \
    | .claude/hooks/post-tool-use/app-javascript-stimulus-guard.sh
  ```
...
- **exit 0 does not mean "passed" — this team's biggest trap.** All four hooks are built for
  ...
  `[Verified]` [Read: .claude/hooks/post-tool-use/app-twig-lint.sh:39]. All four authors must check the
...
- **Exception ① is retired — meta and configuration returned to template ① on 2026-09-11.** Claude Code
  configuration artifacts now run the ordinary agent pair: `utility-claude-code-skill` calls
  `utility-claude-code-author` for a draft at
  `./.claude/tmp/utility/claude-code/artifact-draft.md` (plus its `artifact-meta.txt` sidecar), then
  `utility-claude-code-reviewer` for a `Verdict: PASS` / `Verdict: REDO` on line 1 of
  `artifact-review.md`, writing to the target path only on PASS (max 2 retries, unchanged).
  `[Verified]` 2026-09-13 [Read: .claude/skills/utility-claude-code-skill/SKILL.md]
  > **From 2026-08-08 to 2026-09-11 this was genuinely an exception** — the author and reviewer agents
  > had been merged into the review command and the skill self-verified against it in the main session
  > context. **The criteria never moved**;
  > `skills/utility-claude-code-skill/references/artifact-spec-guide.md` was and remains the sole SoT,
  > now read by the author as writing criteria and by the reviewer as judgment criteria. Only the owner
  > of the verdict changed.
- **Exception ② (agentless variant — providers):** the provider transports run a generate → self-verify
  loop (max 3 retries; the static gates unchanged), owned by **three integration-agnostic skills** —
  `app-src-service-api-{rest,oauth2,websocket}-build-skill`. Each resolves criteria from the target
  integration's rule file when one is registered, and otherwise from the matching
  `app-src-service-api-*-client-skill`. `[Verified]` 2026-09-09
  [Read: .claude/skills/app-src-service-api-rest-build-skill/SKILL.md]
  > **Corrected 2026-09-09 — the previous text and its evidence citation were both stale.** It described
  > five per-provider `*-build` commands under `commands/api/providers/**` and cited one of them as
  > `[Verified]` evidence. **No such file exists or ever did on disk** — the domain shipped organised by
  > transport, not by provider. A `[Verified]` tag pointing at a missing file is worse than no tag.
- **Exception ③ is retired too — shell scripts returned to template ① on the same day.**
  `utility-shell-script-skill` calls `utility-shell-script-author` for a draft at
  `./.claude/tmp/utility/shell-script/script-draft.md` (plus its `script-meta.txt` sidecar), then
  `utility-shell-script-reviewer` for the verdict (max 2 retries). The pair had been retired on
  2026-08-16 and was **restored** on 2026-09-11. `utility-shell-script-rule.md` and
  `utility-shell-script-style.md` remain SoT, and the guide's `## Authoring Conventions` and
  `## Review Procedure` are what the two agents execute. `[Verified]` 2026-09-13
  [Read: .claude/skills/utility-shell-script-skill/SKILL.md]
  > **Only the provider axis (exception ②) is still agentless.** What was once three exceptions is now
  > one; do not cite ① or ③ as evidence for an agentless design.
...
- (±) The self-verifying variant, which gathers the criteria into one file, reduces the artifact count
  (3 → 2) and the drift, but because verification runs in the main session context it **loses the
  isolation of an independent subagent**. **That trade was re-taken on 2026-09-11 and went the other
  way** for meta and configuration: both domains were given agent pairs again, on the judgment that an
  independent verdict context is worth the two extra files. **The provider axis kept the self-verifying
  shape**, because its criteria genuinely converge on one registry-resolved document per integration.
  Read this bullet as a live trade-off with one remaining instance, not as a description of how the
  meta domains work.
...
- **Deploy sub-team (proposed):** **reuses** the domain reviewers as a pre-deploy security and
  ...
  - The `/utility-shell-script-skill` skill → deploy script safety (`rm -rf` guards, etc.).
...
- **Later the same day, this orchestrator's roster closed too** (`app-*` only, by the same explicit
  ...
  `/database-postgresql-skill` that a human closes out. See `## 5.3` below.
...
half and names `/database-postgresql-skill`, `/message-rabbitmq-skill` or `/cache-redis-skill` in
...
### 5.4 Splitting `.claude/**` routing three ways (2026-09-10)

#### Context

`agents/app-agent-team.md` routed **all** `.claude/**` work to two targets: `utility-claude-code-skill`
for authoring and `/utility-claude-code-skill` for review. That list predates
`abstract-agentic-harness-skill`, which was created 2026-09-06 (as three skills) and merged into one on
2026-09-09, and it was never updated to name it.

The gap was one-way in both directions. This team owns `.claude/**` (`## Invariant — scope boundary and
handoffs`), and `harness-design-guide.md` Phase 1 agrees — so **a multi-artifact harness request routed
through this orchestrator landed on the single-artifact skill.** `utility-claude-code-skill` had no
upward boundary sentence either, so once there it had no reason to hand back.

The failure mode is quiet and specific: the single-artifact skill authors its one file correctly and
never sees the orchestrator roster, the `hooks/pre-tool-use/agent-roster-guard.sh` case table or the
`rules/abstract-structure-rule.md` index entry that had to move with it. Every one of those omissions
fails **silently** — a nested `agent-memory/` directory does not load, an out-of-roster spawn is blocked
only at spawn time, a stale `@see` just goes dead.

#### Decision

Route `.claude/**` by **how many artifacts the decision touches**, not by the wording of the request:

| Span | Target |
| --- | --- |
| the artifact **system** — new domain or role axis, roster or routing change, drift audit, non-converging loop | `abstract-agentic-harness-skill` |
| a **single** artifact | `utility-claude-code-skill` |
| **review** of an existing artifact | `/utility-claude-code-skill` |

Recorded in three places in the same change, per `## Companion Updates`: the agent prompt
(`## Cross-domain Handoffs`), the verdict SoT (`rules/app-agent-team-rule.md`,
`## Invariant — scope boundary and handoffs`, where mis-routing is a `[SHOULD]` defect), and the
delegate's own body (`skills/utility-claude-code-skill/SKILL.md`, an upward boundary sentence plus an
`@see`).

#### Consequences

- (+) **The edge is now two-way.** The harness skill already pointed down ("a request that ends at one
  artifact belongs to `utility-claude-code-skill`"); the delegate now points up.
- (+) **No new artifact, no roster change.** Both targets are `Skill` calls, which this team may always
  make — the closed `Agent` roster of section 5.3 is untouched.
- (−) **A judgment call moved onto the router.** "How many files does this span?" is not always obvious
  at request time, and a design that looks single-file can turn out not to be. The mitigation is the
  direction of error: the harness skill delegates back down per file, so over-routing upward costs one
  extra hop, while under-routing loses the companion updates entirely.
...
| 3 | ~~No dedicated agents for the providers~~ | commands/api/providers/finance/** | ✅ **Resolved, then reshaped** — 10 author and reviewer agents were created, then merged into 5 `*-build` commands on 2026-08-15. Generate-verify in the provider domain is handled by the build skills' self-verification loop |
...
- `tools-app-deploy-skill` is not a single author→reviewer loop but a gate that **fans the changed
  ...
  `/utility-shell-script-skill` skill**, consolidates the MUST/SHOULD/CONSIDER findings and returns
