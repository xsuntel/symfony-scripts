...
@see scripts/common/_abstract.sh — global variables / bootstrap source
...
**Common base sourcing order:** `scripts/common/_abstract.sh` → `_environment.sh` → `_platform.sh` →
...
Declared in `scripts/common/_abstract.sh` and available in every script after sourcing. Never re-declare
...
Always confirm the exact names/defaults in `scripts/common/_abstract.sh` — this list is for reference.
...
| --- | --- | --- |
| Base | `scripts/common/` | Environment-independent install/config; sourced by both containers and deploy |
