...
@see .claude/rules/database-postgresql-rule.md — Database judgment criteria (SoT)
...
#[ORM\Table(name: 'account_api_keys')]
...
cd app && php bin/console doctrine:migrations:diff --em=company

# Apply migrations to a specific entity manager
cd app && php bin/console doctrine:migrations:migrate --em=company
