...
| Dedicated review modes | **3** (the Review mode of `/cache-redis-skill` · `/database-postgresql-skill` · `/server-nginx-skill`) | `skills/<n>/references/review-guide.md` |
...
- **`message-rabbitmq-reviewer`** — a real sixth infrastructure reviewer in this repository, but it
  ...
  `/message-rabbitmq-skill` skill. **Do not pull it in because the name looks similar.**
- **A shell-script reviewer** — `utility-shell-script-reviewer` **exists again as of 2026-09-11**, but
  it belongs to `utility-agent-team`, not to this roster: it matches none of the five prefixes. It was
  retired on 2026-08-16, which is why older text here said it did not exist. Route a shell script to
  `/utility-shell-script-skill` (Author or Review mode) — or, in a deploy context, to the deploy gate,
  which fans out to that skill itself.
...
`tools-aws-ecs-reviewer`) plus the `/utility-shell-script-skill` skill, and returns PASS/BLOCK.
...
- **Scalability** (+) each team's spawn targets converge on one prefix family, so fan-out width is
  determined by roster size. (−) with four orchestrators, the boundary description must be
  **maintained across four files together** — each rule's `## Companion updates` section manages that
  cost explicitly.
...
**Create the paired document and restore symmetry across the axes.** The operational contract
document goes back to holding **only what every team shares**, and this team's own inventory,
trade-offs and gaps live here.

#### Consequences

- (+) the shared-slug convention (`agents/`, `rules/`, `docs/` carrying the same slug) now holds for
  every orchestrator, so every `@see` resolves without exception.
- (+) removing this team's specifics from the shared contract document sharpens its character as
  genuinely **common to all of them**.

> **This decision was taken on 2026-08-30, when there were three teams; it now covers four.**
> `utility-agent-team` (added 2026-09-11) gained its paired `rules/utility-agent-team-rule.md` and
> `docs/utility-agent-team-docs.md` on 2026-09-13, which is this same decision applied a fourth time
> rather than a new one. The wording above was de-numbered on that date so the next orchestrator does
> not require a fifth edit.
...
### 6.1 There are no dedicated GCP and AWS review modes

`/cache-redis-skill`, `/database-postgresql-skill` and `/server-nginx-skill` exist; Cloud Run and
...
| The 3 standalone review modes | `.claude/skills/{cache-redis,database-postgresql,server-nginx}-skill/references/review-guide.md` |
