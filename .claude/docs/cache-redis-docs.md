...
@see .claude/rules/cache-redis-rule.md — Redis/Cache judgment criteria (SoT)
...
- Lock key pattern for API rate limiting: `lock_{provider}_{endpoint_tr_id}`.

**Messenger sync transport** — declared in `app/config/packages/messenger.yaml`, each pointing at
`MESSENGER_TRANSPORT_DSN_REDIS`.
