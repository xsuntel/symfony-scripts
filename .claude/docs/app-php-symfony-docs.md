...
- **Subscribe** — the open-orders list subscribes to a single URI-template topic (`trading/orders/{provider}/{id}`) with
...
an implementation, review each PHP file with the `/app-php-symfony-skill {file}` command; the judgment criteria's
...
| `supports` | The `...\App\Trading\Orders` entities |
...
$notification = (new Notification('Provider OAuth2 authentication failed', ['email']))
    ->content('Check the validity of the client credentials and whether the key needs rotation.');
